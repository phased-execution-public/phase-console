...
export function Controls({
  ...
  live: boolean;
  /**
   * The lanes this console can still see output from (`RunDetail.liveness[]`).
   *
   * `live` above is `isLiveStatus(run.status)` — a word the run wrote about
   * itself. This is the other kind of fact, and the reason `resumable` needed
   * it: a run whose status says `halted` or `parked` can still have a session
   * editing the tree, and Continue over that is a second agent on one working
   * tree. The server refuses it now; this is the same refusal shown BEFORE the
   * press instead of as a toast after it.
   */
  liveness?: LaneLiveness[] | undefined;
  /**
   * The run page's own busy word, for everything that is NOT a lifecycle verb.
   *
   * The four verbs below carry their own busy state now (`useRunLifecycle`),
   * so this no longer decides whether Freeze reads "Freezing…" — it only keeps
   * the whole card disabled while the page is doing something else.
   */
  busy: string;
  allowRun: boolean;
  ...
  // The four verbs go through the ONE hook, with no phase: these are the
  // whole-run controls, and a run-level freeze names no phase by design (the
  // per-session version lives on each lane's tab). One busy shape, one toast
  // vocabulary, one invalidation set — see `lib/run-lifecycle.ts`.
  const lifecycle = useRunLifecycle(slug);
  // A lane with recorded liveness has a phase record in flight — a session
  // that has not stopped. Only meaningful when the status word disagrees;
  // while `live` is true it is saying the same thing twice.
  const stillWorking = !live && (liveness?.length ?? 0) > 0;
  const resumable = Boolean(run) && !live && !stillWorking && run?.status !== 'finished';
  const disabled = !allowRun || Boolean(busy) || lifecycle.busy != null;
    ...
    <Card>
      <CardHeader className="flex-wrap items-baseline">
        ...
        <span className="max-w-prose text-2xs text-ink-faint">
          {frozen
            ...
            : stillWorking
              ? 'The run has stopped but a session has not. Stop it, or wait for it — continuing now ' +
                'would put a second agent on the same working tree.'
              : resumable
                ? checkpointed
      ...
      <CardBody className="flex flex-col gap-3">
        ...
        {live && (
          <div className="flex flex-wrap items-center gap-2">
            {pausing ? (
              <Button variant="action" disabled={disabled} onClick={() => void lifecycle.resume()}>
                {lifecycle.busy === 'resume' ? 'Cancelling…' : 'Cancel pause — keep going'}
              </Button>
            ) : (
              <Button
                disabled={disabled || stopping || halting}
                // The toast reports what the SERVER did, not what the click
                // intended — a pause that lands on nothing answers 200 and
                // would otherwise read as "it worked". That reading lives in
                // `lifecycleToast` now, where all seven verbs share it.
                onClick={() => void lifecycle.pause()}
              >
                {lifecycle.busy === 'pause' ? 'Arming…' : 'Pause after this phase'}
              </Button>
            )}

            {frozen ? (
              <Button variant="action" disabled={disabled} onClick={() => void lifecycle.thaw()}>
                {lifecycle.busy === 'thaw' ? 'Continuing…' : 'Continue the frozen session'}
              </Button>
            ) : (
              <Button
              ...
              >
                {lifecycle.busy === 'freeze' ? 'Freezing…' : 'Freeze now'}
              </Button>
            )}

            <StopButton
              disabled={disabled}
              busy={lifecycle.busy === 'stop'}
