...
test('a watcher error is survived and re-armed, not thrown', async () => {
  ...
  try {
    ...
    //
    // 🔴 The stimulus REPEATS, and that is the fix — not a bigger number. This
    // wrote the file ONCE and then waited, so it was not measuring how long the
    // event takes: `fs.watch` on macOS has a window after it returns in which
    // events are simply LOST, and no ceiling recovers one. Raising 5s to 20s
    // moved the failure to 21s; raising it to 60s moved it to 61s, which is how
    // the mechanism gave itself away. The claim was always "the re-armed watch
    // DELIVERS EVENTS", never "it delivered the one event I wrote at exactly
    // this instant" — so the write is retried until one lands. A watch that
    // never delivers still fails, now in 20s and for the right reason.
    seen.length = 0;
    const target = join(dir.plans, 'after-rearm.md');
    let delivered = false;
    const deadline = Date.now() + 20_000;
    while (!delivered && Date.now() < deadline) {
      writeFileSync(target, `# ${Date.now()}\n`);
      delivered = await until(() => seen.some((p) => p.includes('after-rearm.md')), 500);
    }
    assert.ok(delivered, 'the re-armed watch should deliver events');
  } finally {
    watcher.stop();
    clearDegraded('watcher');
    dir.cleanup();
