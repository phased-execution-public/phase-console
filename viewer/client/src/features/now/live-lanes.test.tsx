...
describe('a lane row', () => {
  ...
  /**
   * Two runs of one repository, both live, both on phase 10: the rows read as
   * the same sentence twice. The branch chip is the only thing on either that
   * says which checkout the commits are landing in — and it appears exactly
   * when the lane HAS one of its own, because a `ChildRef.branch` is set only
   * alongside `worktree` and absent means the run's own branch.
   */
  it('wears the lane’s own branch when it has one', () => {
    mount({
      lanes: [
        lane({
          child: {
            ...
            worktree: '/state/wt/demo',
            branch: 'pe/demo-p4',
          },
        }),
      ],
    });
    expect(screen.getByTestId('branch-chip').textContent).toContain('pe/demo-p4');
  });

  it('wears no branch chip for a lane sharing the run’s own checkout', () => {
    mount({ lanes: [lane()] });
  ...
  it('explains a lane that is not running instead of leaving "queued" bare', () => {
    ...
    // The wait is a live `<RelativeTime>` now — the whole point of the line is
    // how long it has been — so the sentence spans two nodes and the row is
    // what has to be read, not one text node.
    const queued = screen.getByTestId('lane-row');
    expect(queued.textContent).toMatch(/Queued behind another owner’s claim since .*30 min/);
