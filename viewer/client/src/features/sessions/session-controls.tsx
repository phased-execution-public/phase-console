...
import { Button, ButtonGroup, ConfirmButton, toast } from '@/components/ui';
import { api, type TerminalSession, type TerminalState } from '@/lib/api';
import { elapsed } from '@/lib/format';
import { keys, toastError } from '@/lib/queries';

/**
 * What a freeze IS, and what happens to it — the card's whole job.
 *
 * "Continues mid-token, in the same process" was only the first half, and the
 * half that matters less: a freeze is HELD, so the question it raises is what
 * ends the holding. `escalateAt` is that answer, and the same one the lane card
 * makes, because both are the same clock (`freeze.ts`). A record written before
 * the deadline existed simply keeps the old sentence rather than inventing a
 * time it cannot know.
 */
export function frozenTitle(frozen: NonNullable<TerminalSession['frozen']>, now = Date.now()): string {
  const held = `Frozen by ${frozen.by}, ${elapsed(now - frozen.at)} ago — continues mid-token, in the same process`;
  const at = Date.parse(frozen.escalateAt ?? '');
  if (!Number.isFinite(at)) return held;
  return at <= now
    ? `${held}. Past its deadline — it is being stopped now.`
    : `${held}. Left frozen past ${new Date(at).toLocaleTimeString()} (${elapsed(at - now)} from now) it is stopped gracefully.`;
}
  ...
  return (
    <ButtonGroup aria-label={`${session.label} session controls`}>
      ...
      <ConfirmButton
        ...
        className={touch}
        disabled={disabled || Boolean(session.stopping)}
        busy={busy === 'stop' || Boolean(session.stopping)}
        busyLabel="Stopping…"
