...
import { FOCUS_KEYS, parseHash, redirectTarget, type FocusKey, type Route } from '@/app/routes';
...
async function mount(hash = '#/now') {
  ...
  return render(
    // Deliberately bare: the page is mounted without the app shell here and by
    // `features/a11y.test.tsx`, so anything on it that needs a provider has to
    // bring its own (the strip's `InfoTip` does).
...
describe('the addresses it absorbed', () => {
  ...
  /**
   * `FOCUS_KEYS` is declared closed, with the reason written above it: "a value
   * the page did not recognise would be a silently ignored deep link — the same
   * defect class as `#/plan/x/autopilot`". `plans` was exactly that value, for
   * the whole life of the vocabulary: `focusOf()` returned it and no surface on
   * Now had ever heard of it.
   *
   * So the vocabulary is pinned to its effect, not to its own definition. A
   * fifth key added without a band fails here rather than shipping inert.
   */
  it('gives EVERY FOCUS_KEYS value a band that observes it', async () => {
    const bands: Record<FocusKey, string> = {
      inbox: 'needs-you',
      lanes: 'live-lanes',
      next: 'next-up',
      plans: 'plans-in-flight',
    };
    // The map is the vocabulary — no key may be missing from it.
    expect(Object.keys(bands).sort()).toEqual(Object.values(FOCUS_KEYS).sort());

    // `inbox` is in here now. It used to be skipped — "the inbox takes focus,
    // not a mark" — and that exemption WAS the defect: the band called
    // `listRef.focus()` and never scrolled, so `#/now?focus=inbox` landed at
    // the top of the page on a phone exactly as a bare `#/now` does. Every one
    // of the four goes through `useFocusBand` now, so every one of the four is
    // marked, and the exemption that hid one of them is gone.
    for (const [key, testId] of Object.entries(bands)) {
      const { unmount } = await mount(`#/now?focus=${key}`);
      const band = await screen.findByTestId(testId);
      expect(band.getAttribute('data-focused'), key).toBe('true');
      unmount();
    }
  });

  it('hands the inbox band the keyboard as well as the scroll', async () => {
    await mount(`#/now?focus=${FOCUS_KEYS.inbox}`);
    const list = await screen.findByRole('list', { name: 'Things that need you' });
    // Both halves of what the address means: be here, and be ready to triage.
    expect(document.activeElement).toBe(list);
    expect(screen.getByTestId('needs-you').getAttribute('data-focused')).toBe('true');
  });
});
