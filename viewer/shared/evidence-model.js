 ...
 *  9. **"Nothing is running it" vs "nobody looked."** `stale` is the second
 *     half of the same sentence this module exists for: `board: in-progress`
 *     is a CLAIM (a `status: in-progress` line in a handoff), and `live` is
 *     whether anything was actually observed behind it. The two are told apart
 *     by the input, and the distinction is load-bearing, so read it carefully:
 *
 *       `live` ABSENT   the caller has no liveness dependency — `stale` stays
 *                       false and nothing is said. The drive loop is this
 *                       caller (`situation.ts` has its own, richer evidence).
 *       `live: null`    the caller LOOKED and found nothing. This is what
 *                       makes `stale` true.
 *       `live: {…}`     something was observed; `via` says which witness.
 *
 *     A caller that means "I looked and found nothing" must pass `null`
 *     explicitly — `...(fact ? { live: fact } : {})` says "I did not look",
 *     which is the conservative answer and therefore the silent one. A warn
...
// The board's bucket vocabulary is OWNED by `status-vocab.js`; `BOARD_WORDS`
// below is that list plus `unknown`, imported rather than re-typed.
import { BOARD_BUCKETS } from './status-vocab.js';
import { HANDOFF_STATUS_WORDS, QA_DISPLAY_WORDS } from './plan-vocab.js';
...
/**
 * @typedef {(typeof QA_DISPLAY_WORDS)[number]} QaWord
 */
 ...
 */
export const QA_WORDS = QA_DISPLAY_WORDS;

/**
 * @typedef {(typeof BOARD_WORDS)[number]} BoardWord
 */

/**
 * The engine's five bucket words plus `unknown`, which is what a board ERROR
 * reads as. The five come from `status-vocab.js`'s `BOARD_BUCKETS` — the one
 * owner — so this stays a deliberate SUPERSET rather than a drifting copy.
 * @type {readonly BoardWord[]}
 */
export const BOARD_WORDS = Object.freeze([...BOARD_BUCKETS, 'unknown']);

/**
 * @typedef {(typeof HANDOFF_WORDS)[number]} HandoffWord
 */
 ...
 */
/**
 * The order this module lists handoff words in; MEMBERSHIP is the owner's
 * (`plan-vocab.js`). Declared before `HANDOFF_WORDS` because that list reads
 * it while initialising — a `const` below it is a temporal-dead-zone crash at
 * module load, which takes every importer of this module down with it.
 */
const HANDOFF_DISPLAY_RANK = ['pending', 'in-progress', 'blocked', 'complete', 'unknown'];

export const HANDOFF_WORDS = Object.freeze([
  'absent',
  ...[...HANDOFF_STATUS_WORDS].sort(
    (a, b) => HANDOFF_DISPLAY_RANK.indexOf(a) - HANDOFF_DISPLAY_RANK.indexOf(b),
  ),
]);
 ...
 */
export { RULING_KINDS } from './attention-model.js';

/**
 * @typedef {'run'|'lock'|'registry'} LiveVia
 */

/**
 * The three witnesses that can say a phase is being worked RIGHT NOW, in
 * falling authority — see `LiveFact`. One list, here, because server, client
 * and tests all switch on it: three copies that agree today are three copies
 * that disagree the day a fourth witness is added.
 */
export const LIVE_VIA = Object.freeze(
  /** @type {const} */ ([
    /** A run record in flight on a settled read — this console's loop, or a probed process. */
    'run',
    /** An unexpired phase lock whose own session the registry vouches for. */
    'lock',
    /** The phase's session is live in the registry though its run record was settled. */
    'registry',
  ]),
);

/** @param {unknown} value */
export function isLiveVia(value) {
  return typeof value === 'string' && /** @type {readonly string[]} */ (LIVE_VIA).includes(value);
}

/** How a witness reads in prose. @param {LiveVia} via */
function liveSource(via) {
  if (via === 'run') return 'a run this console can see';
  if (via === 'lock') return 'a lock whose session is live';
...
/**
 * Something OBSERVED to be working this phase right now, in falling order of
 * authority — the fact the board word is a claim about.
 *
 *   `run`       a run record for this phase is in flight on a settled read:
 *               either this console's own loop is driving it, or a child
 *               process the probe says still holds work outlived a console.
 *               The strongest, because a process is a fact.
 *   `lock`      an unexpired phase lock whose own `session=` the session
 *               registry positively vouches for. Stronger than a run RECORD
 *               (which is another claim) and weaker than a probed process.
 *   `registry`  the phase's session is live in the registry although its run
 *               record has been settled — the case where the two probes
 *               disagree and the registry's own heartbeat is the fresher fact.
 *
 * @typedef {object} LiveFact
 * @property {'run'|'lock'|'registry'} via Which witness saw it.
 * @property {string} [session] The Claude session id, when the witness knows one.
 * @property {number} [pid] The process, when the witness knows one.
 */
 ...
 * @property {LiveFact|null} [live] Ambiguity 9 — `null` is "looked, found nothing"; ABSENT is "did not look".
 */
 ...
 * @property {boolean} stale The board paints work in flight and nothing live was found behind it.
...
/**
 * Ambiguity 9, decided by PRESENCE of the key rather than by its value:
 * `{live: null}` is a measurement, `{}` is not one. `hasOwnProperty` and not
 * `!== undefined`, so that an explicit `{live: undefined}` — which is what a
 * conditional spread collapses to — reads as "did not look" too.
 *
 * A key that is present but unreadable is still a measurement, and it found
 * nothing: an unrecognised `via` names no witness, so it cannot vouch for
 * anything. `why[]` quotes it so the caller can be fixed.
 *
 * @param {EvidenceInput} inp
 * @returns {{checked: boolean, fact: {via: LiveVia, session: string, pid: number|null}|null, bad: string}}
 */
function readLive(inp) {
  if (!Object.prototype.hasOwnProperty.call(inp, 'live') || inp.live === undefined) {
    return { checked: false, fact: null, bad: '' };
  }
  const raw = inp.live;
  if (!raw || typeof raw !== 'object') return { checked: true, fact: null, bad: '' };
  const via = String(raw.via ?? '');
  if (!isLiveVia(via)) return { checked: true, fact: null, bad: tidy(via, 40) || '(empty)' };
  const pid =
    typeof raw.pid === 'number' && Number.isFinite(raw.pid) && raw.pid > 0 ? Math.floor(raw.pid) : null;
  return {
    checked: true,
    fact: { via: /** @type {LiveVia} */ (via), session: tidy(raw.session, 64), pid },
    bad: '',
  };
}
...
export function deriveEvidence(input) {
  ...
  if (board === 'unknown') {
    ...
    // No claim about work in flight can be stale, because no claim was read.
    return { board, handoff, verification, qa, evidenced: false, stale: false, why };
  }
  ...
  // `in-progress` alone, not `midWork`. Only ONE board word paints a phase as
  // running — `BOARD_STATE_UI['in-progress'] === 'running'` — and it is the
  // word `phase_status()` derives from a `status: in-progress` line in a
  // markdown file. That is the claim this measures. `stuck` is deliberately
  // excluded: it claims a PERSON is needed, not that work is in flight, so
  // badging it "claimed running" would put a second warning on a state that is
  // already a warning. The incident: a phase read "Running" for 18 days with
  // no run, no lock and no process.
  const claimsRunning = board === 'in-progress';
  const stale = claimsRunning && live.checked && !live.fact;
  if (live.bad)
    why.push(`live: the witness "${live.bad}" is not one of ${LIVE_VIA.join('/')} — read as nothing live`);
  if (live.fact) {
    const who = live.fact.session ? ` (session ${live.fact.session})` : '';
    const pid = live.fact.pid ? ` pid ${live.fact.pid}` : '';
    why.push(`live: ${liveSource(live.fact.via)}${who}${pid}`);
  } else if (stale) {
    why.push('live: nothing is running this — no run, no live lock, no session. the board word is a claim');
  } else if (claimsRunning && !live.checked) {
    why.push('live: not checked here');
  }
  ...
  // `stale` is NOT folded into `evidenced`. They answer different questions
  // about different phases — `evidenced` is about a `done` claim and its
  // proof, `stale` about an `in-progress` claim and its process — and the two
  // sets do not overlap (a phase is one board word at a time). Folding them
  // would make one warn chip mean two things, which is how a surface grows a
  // case nobody styled.
  return { board, handoff, verification, qa, evidenced, stale, why };
}
