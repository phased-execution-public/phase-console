...
describe('lanesOf', () => {
  ...
  it('tabs a done phase while a QA round is in flight on it, and names the round', () => {
    // A review happens AFTER the work, so the board and the record both read
    // `done` for the whole of it — and `done` is not a lane status. The runner
    // marks the record while the reviewer runs; without this the session the
    // run was paying for had no pane, no console and no task list anywhere.
    const lanes = lanesOf(
      run({
        phases: {
          ...phases({ phase: 2, status: 'done' }),
          '3': {
            phase: 3,
            status: 'done',
            attempts: 1,
            ...
            qaSession: {
              round: 2,
              report: 'reports/phase-03-qa-round2.md',
    ...
    expect(lanes[0]!.qa).toEqual({ round: 2 });
...
describe('QueuedPane', () => {
  ...
  /**
   * The wait, read in reverse.
   *
   * The scheduler carves branch-disjoint claims out of admission entirely, so a
   * holder that names a branch here is one whose branch was NOT disjoint from
   * this entry's. Until this rendered, "why is this queued when both runs have
   * their own checkout" had no answer on the page at all — the scheduler had
   * carried `Holder.branch` since phase 8 and the client type dropped it.
   */
  it('names the branch a holder is on, when it declared one', () => {
    mount(<QueuedPane phase={4} entry={entry({ waitingOn: [holder({ branch: 'pe/other-plan' })] })} />);
    expect(screen.getByTestId('holder-branch').textContent).toContain('pe/other-plan');
  });

  it('says nothing about a branch for an unqualified holder — absent is not a gap', () => {
    mount(<QueuedPane phase={4} entry={entry({ waitingOn: [holder()] })} />);
    expect(screen.queryByTestId('holder-branch')).toBeNull();
  });
