...
/**
 * The S/M/L weights, READ from `scripts/sizing.env` rather than restated.
 *
 * These were three literals — a fourth copy of numbers F5 says live in one
 * place (coverage-6), in the suite that reasons hardest about weights and so
 * the one least likely to notice them moving. Deriving them means a change to
 * sizing.env reaches this file for free; nothing here depends on the particular
 * values, only on S < M < L.
 */
