...
export function AskBox({
  ...
  const send = async () => {
    ...
    try {
      // The PHASE is part of the message, and leaving it off was not a
      // simplification. The server addresses a named lane and falls back to
      // `mirrorLane()` — the LOWEST-numbered live lane — when nobody names one.
      // With phases 9 and 21 both running, a course correction typed under 21,
      // in a box whose own placeholder says "the session running phase 21",
      // was delivered to phase 9's session, which then acted on it confidently
      // and about the wrong work. The guard existed on the server the whole
      // time; this is the client finally using it.
      const target = phase ?? undefined;
      const sent = steering
        ? await api.runSteer(slug, body, key, target)
        : await api.runAsk(slug, body, key, target);
      setText('');
    ...
    <div className={cn('border-t border-rule px-3 py-2', steering && 'bg-action/6')}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          className={cn(field, 'shrink-0 text-xs')}
        >
