...
test('a real shell runs and reattaches — or says why it cannot, leaving the console intact', async (t) => {
  ...
  // 🔴 Sent AGAIN until it lands, rather than once into a shell that may still
  // be starting. The operator's real login shell runs here — an interactive zsh
  // with whatever prompt and plugins they have — and under full-suite load its
  // startup has outrun the 8s deadline with the keystrokes swallowed before it
  // began reading: the captured tail was the prompt still painting. `echo` is
  // idempotent, so re-sending costs nothing, and the claim is unchanged — the
  // shell must run the command. Note `42` appears only AFTER expansion, so the
  // shell's own echo of the typed line cannot satisfy this.
  let ran = false;
  const sendUntil = Date.now() + 30_000;
  while (!ran && Date.now() < sendUntil) {
    first.socket.send(JSON.stringify({ t: 'i', d: 'echo MARKER_ONE_$((6*7))\r' }));
    ran = await until(() => first.out().includes('MARKER_ONE_42'), 2_000);
  }
  assert.ok(ran,
