...
export function PlanCard({ row, index }: { row: PlanRow; index: number }) {
  ...
  return (
    <Card
      className="animate-fade px-(--pad-x) py-(--pad-y) transition-colors hover:border-rule-strong"
      // Capped so a long list does not spend two seconds arriving.
      style={{ animationDelay: `${Math.min(index, 10) * 18}ms` }}
    >
      {/* `flex-wrap` and a floor on the name, together — one without the other
          fixes nothing. At 360 the badge cluster (`1 waiting` 100px + `parked`
          76px) is `shrink-0` and the plan's name was `min-w-0`, so the name
      ...
      <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1.5">
        ...
        <span className="ml-auto flex min-w-0 max-w-full items-center gap-2 text-2xs text-ink-faint">
          ...
          <RelativeTime at={row.activity} live={false} className="shrink-0 whitespace-nowrap" />
        </span>
      </div>
