...
test('sleep is pacing the runner executes, not a check for a person', () => {
  // "docker compose up …; sleep 8; task test" is the standard preamble of a
  // containerised plan — and `sleep` unrecognised put "a check only you can
  // make" card on EVERY phase of a real trusted run, one of which held ~6h
  // overnight for a person to vouch for a pause.
  assert.deepEqual(runs('sleep 8'), ['sleep 8']);
  assert.equal(held('sleep 8').length, 0);
  // Judged per segment: the pause runs, the payload behind it is still judged
  // on its own.
  const mixed = only('`sleep 5 && npm test`');
  assert.deepEqual(mixed.commands, ['sleep 5 && npm test']);
});
