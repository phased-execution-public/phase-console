...
describe('auto-grant', () => {
  const withAutoApprove = (value: { global: boolean | null; plan: boolean | null; effective: boolean }) => {
    (POLICY as Record<string, unknown>).autoApprove = value;
  };
  afterEach(() => {
    delete (POLICY as Record<string, unknown>).autoApprove;
  });

  it('says nothing about auto-grant on a server that predates it', async () => {
    await mount();
    expect(screen.queryByText('Auto-grant approvals')).toBeNull();
  });

  it('shows the shipped default as On and flips the global scalar in one edit', async () => {
    withAutoApprove({ global: null, plan: null, effective: true });
    await mount();
    const button = screen.getByRole('button', { name: /On \(the default\)/ });
    expect(button.getAttribute('aria-pressed')).toBe('true');
    fireEvent.click(button);
    await waitFor(() =>
      expect(editPolicy).toHaveBeenCalledWith(
        expect.objectContaining({ scope: 'global', autoApprove: false }),
      ),
    );
  });

  it('the plan scope offers inherit/on/off and clears with null', async () => {
    withAutoApprove({ global: false, plan: true, effective: true });
    await mount();
    // Switch to the plan scope and pick a plan.
    fireEvent.change(screen.getByDisplayValue('everywhere'), { target: { value: 'plan' } });
    // No plans in the mock list — the tri-state needs a slug, so assert the
    // global control is gone and nothing rendered a select for the plan yet.
