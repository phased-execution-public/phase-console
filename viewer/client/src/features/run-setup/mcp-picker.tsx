...
export function McpPicker({
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer flex-wrap items-center px-2 py-1.5 text-sm [@media(hover:none)]:min-h-(--tap-min)">
      ...
      <div className="grid gap-1.5 border-t border-rule p-2">
        ...
        {ordered.map((server) => {
          ...
          return (
            <label
              key={server.id}
              // `tap-row`: a row in a stack of rows takes the thumb floor as
              // its box — the checkbox's own overlay would reach into its
              // neighbours (`docs/design.md` §6.1).
              className={`tap-row flex items-start gap-2 text-sm ${ok || fromPlan ? '' : 'opacity-60'}`}
            >
              <Checkbox
                className="mt-1"
                checked={fromPlan || chosen.includes(server.id)}
                // A broken server may never be newly ticked — but one already
                // ON the run must stay untickable-OFF. `disabled` used to read
                // usability alone, so a server whose token expired overnight
                // rendered ticked AND greyed: the run could not be continued
                // without it and could not be continued with it either, and
                // the only way out was to start a fresh run.
                disabled={fromPlan || (!ok && !chosen.includes(server.id))}
              ...
              <span className="min-w-0">
                ...
                {server.toolCount ? (
                  <span className="ml-1.5 text-2xs text-ink-muted">{server.toolCount} tools</span>
                ) : null}
              </span>
            </label>
          );
