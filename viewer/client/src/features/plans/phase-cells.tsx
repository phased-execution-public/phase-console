 ...
 *
 * ---- Why every cell is memoised ----
 *
 * A board is a wide table drawn once per phase, and the events that redraw it
 * are almost always news about ONE phase: a lock taken, a handoff written, a
 * lane starting. Without `memo`, one such event re-rendered every cell of every
 * row — including a `MarkdownInline` per title, which is a parser and a DOM
 * sweep, and a `ScopeChips` per row.
 *
 * The memo only pays because the props are referentially stable across such an
 * event, and they are for one reason worth naming: TanStack Query's
 * `structuralSharing` is on (it is the default and `lib/queries.ts` does not
 * turn it off), so a refetch that changes one phase returns the SAME object for
 * every phase it did not change. Turn structural sharing off and every memo on
 * this page silently stops paying — the cells stay correct, just uniformly
 * slower, with nothing failing to say so.
 */
 ...
 * paint alike.
 */
export const LockChip = memo(function LockChip({
  lock,
  compact = false,
}: {
  lock?: PhaseLock;
...
export const LockCell = memo(function LockCell({ lock, compact }: { lock?: PhaseLock; compact?: boolean }) {
 ...
 */
export const DepsCell = memo(function DepsCell({
  slug,
  phase,
  linked = true,
  max = 6,
  ...
  linked?: boolean;
  /**
   * How many dependencies are drawn before the rest become a count.
   *
   * A late phase in a long plan can depend on twenty others, and twenty chips
   * in a nine-rem column stack into a five-hundred-pixel row — one phase
   * taller than the six around it put together, for a fact ("it is last")
   * that the number says better than the list does.
   */
  max?: number;
}) {
    ...
    <div className="flex flex-col gap-1">
      {needs.length > 0 && (
        <div className="flex flex-wrap items-center gap-1">
          ...
          {linked ? (
            <>
              {needs.slice(0, max).map((dep) => (
                <PhaseLink key={dep} slug={slug} phase={dep} />
              ))}
              {needs.length > max && (
                <span
                  className="font-mono text-2xs text-ink-faint"
                  title={`Also needs ${needs
                    .slice(max)
                    .map((d) => `P${d}`)
                    .join(', ')}`}
                >
...
export const SizeCell = memo(function SizeCell({
  phase,
  eta,
}: {
  phase: PhaseView;
 ...
 * column.
 */
export const FlagsCell = memo(function FlagsCell({
  slug,
  phase,
  showQa = true,
  linked = true,
  ...
  if (phase.review) {
    chips.push(
      <ReviewVerdictChip
        key="review"
        review={phase.review}
        {...(linked ? { href: phaseHref(slug, phase.phase) } : {})}
      />,
    );
  }
  if (phase.handoff) {
...
export const TitleCell = memo(function TitleCell({ phase }: { phase: PhaseView }) {
  return (
    <>
...
export const ScopeCell = memo(function ScopeCell({
  phase,
  conflicts,
}: {
  phase: PhaseView;
      ...
      {lock && (
        <KeyValue
          items={[
            ...
            ['Claimed', lock.claimedAt ? <RelativeTime at={lock.claimedAt} /> : null],
      ...
      {h && (
        <KeyValue
          items={[
            [
              'Handoff',
              <a key="f" href={handoffHref(slug, phase.phase)} className="hover:underline">
                {h.title || h.file}
              </a>,
            ],
