...
/**
 * D19 — a frozen pty session has an OWNER: the card says what happens and when,
 * and then it happens.
 *
 * The lane freeze grew a durable 15-minute escalation after a session outlived
 * its console by three and a half hours. The pty freeze kept the original
 * defect exactly: SIGSTOP, a record, and nothing anywhere that ever came back
 * to it — a session frozen and forgotten stayed stopped for as long as the
 * console lived, holding its memory and its working tree.
 *
 * The window is a parameter here, the way `stop`'s grace is; the DECISION is
 * `freezeVerdict`, shared with lanes, so the two cards cannot promise different
 * things.
 */
test('a frozen session escalates to a graceful stop at its own deadline', async () => {
  const { terminals, signals } = registry();
  const id = await mintOne(terminals);

  terminals.freeze(id, 'the tab strip', 20);
  const info = terminals.state().sessions.find((s) => s.id === id);
  assert.ok(info?.frozen?.escalateAt, 'the promise is on the record, so the card can draw it');
  assert.ok(
    Date.parse(info!.frozen!.escalateAt) > Date.now() - 1_000,
    'and it is a readable, future deadline rather than a decoration',
  );

  await sleep(80);
  // SIGCONT before SIGTERM — the escalation goes through `stop()`, because a
  // stopped process is never scheduled to act on a signal.
  assert.deepEqual(
    signals, [[4242, 'SIGSTOP'], [4242, 'SIGCONT'], [4242, 'SIGTERM']],
    'the freeze converted itself instead of standing forever',
  );
  assert.equal(
    terminals.state().sessions.find((s) => s.id === id)?.frozen, undefined,
    'and the record no longer claims to be merely held',
  );
});

test('a thawed session is not escalated afterwards — the clock came off with the freeze', async () => {
  const { terminals, signals } = registry();
  const id = await mintOne(terminals);

  terminals.freeze(id, 'console', 20);
  terminals.thaw(id);
  await sleep(80);

  assert.deepEqual(
    signals, [[4242, 'SIGSTOP'], [4242, 'SIGCONT']],
    'no SIGTERM: thawing is the operator taking the freeze back, not deferring it',
  );
});
