...
export default function SessionsView({ route }: { route: Route }) {
  ...
  // Pending cards mark THEIR lane "needs permission" on the list.
  ...
  const rows = useMemo(
    () => sessionRows({ terminals: terminals?.sessions, lanes, foreign: registry?.sessions, approvals }),
    [terminals, lanes, registry, approvals],
  );
...
function SessionsList({
  ...
  // The search term is component state and the kind is a preference, the same
  // split the fleet makes: a chip is a view you left set, a search term is a
  // question you were asking a moment ago and would be startled to find still
  // narrowing the list tomorrow.
  const [local, setLocal] = useState<Filters>(NO_FILTERS);
  const filters: Filters = useMemo(
    () => ({ ...local, kind: prefs.sessionsKind ?? '' }),
    [local, prefs.sessionsKind],
  );
  const sortId: SortId = isSortId(prefs.sessionsSort) ? prefs.sessionsSort : 'activity';
  // Counts are of everything, not of what survives the filter: a chip that
  // counted the filtered set would read 0 for every kind but the chosen one.
  const counts = useMemo(() => kindCounts(rows), [rows]);
  const shown = useMemo(() => sortRows(applyFilters(rows, filters), sortId), [rows, filters, sortId]);

  const onFilters = (patch: Partial<Filters>) => {
    const { kind, ...rest } = patch;
    if (kind !== undefined) setPrefs({ sessionsKind: kind });
    ...
    <div className="h-full min-h-0 overflow-y-auto">
      ...
      >
        <div className="flex flex-col gap-3">
          {/* The controls exist only where there is something to control: one
              shell and nothing else is a list a filter cannot improve. */}
          {rows.length > 1 && (
            <Controls
              sortId={sortId}
              onSort={(id) => setPrefs({ sessionsSort: id })}
              filters={filters}
              onFilters={onFilters}
              grouped={prefs.sessionsGroup}
              onGrouped={(value) => setPrefs({ sessionsGroup: value })}
              counts={counts}
            ...
            empty={
              rows.length > 0 ? (
                <Empty
                  ...
                  body="No session of that kind, or nothing with those words in its name, directory or id."
                  action={
                    <Button size="sm" onClick={() => setPrefs({ sessionsKind: '' })}>
                      Show every kind
                    </Button>
                  }
                />
