...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // Answering, in the two shapes a payload can arrive in.
      //
      // The token path is the one this console mints now: the payload names an
      // inbox item and the verbs it authorises, and ONE route decides what that
      // means (`server/push/actions.ts`). The `approvalId` path below it is what
      // a payload minted before action tokens existed carries — and an
      ...
      if (answer) {
        let answered = false;
        try {
          const response = await fetch(answer.url, answer.init);
        ...
        if (answered) {
          ...
          const [title, options] = answeredNotification(verb ?? decision ?? '', info);
          await self.registration.showNotification(title, options);
          return;
        }
        // It did not land — the item may have been cleared, the token may have
        // expired, or this console may have been started without the capability
        // the action needs. Open the console so it can be seen and answered
        // there, which is the behaviour every button had before this existed.
      }
