...
/*
 * The departures board is windowed, so it needs a scroller with a height.
 *
 * jsdom lays nothing out, and `@tanstack/virtual-core` treats an outer size of
 * zero as "no range at all" and yields NO rows — so an unstubbed board renders
 * a header, a total height and nothing else, and every assertion about a row
 * fails for a reason that has nothing to do with the row. `offsetHeight` is
 * what the engine measures the scroller with; row heights are deliberately
 * left at jsdom's zero here, which puts every row of these small fixtures in
 * the DOM and keeps these tests about the plan surface rather than about the
...
describe('the plan surface renders its parts', () => {
  /*
   * The way out, and it is not decoration. A plan is reachable from a push
   * notification, a handoff link and the palette, so this page is routinely
   * the first thing a session sees — and its only exits were the tab bar,
   * which highlights *Plans* without going to the list, and the browser's
   * Back, which a deep link has nothing to go back to.
   */
  it('offers the way back to the list, on every width', async () => {
    renderPlan(['demo']);
    const back = await screen.findByRole('link', { name: '← All plans' });
    expect(back).toHaveAttribute('href', '#/plans');
    // Not hidden above a breakpoint: arriving from a deep link on a laptop is
    // the same dead end as arriving from one on a phone.
    expect(back.className).not.toMatch(/lg:hidden|hidden/);
  });
  ...
  /* ---------------- money on the page somebody is already on ---------------- */

  it('puts the total and the finish date in the header, linking to the full breakdown', async () => {
    ...
    renderPlan(['demo']);
    const link = await screen.findByRole('link', { name: '$61.50' });
    // Not a dead figure: the header is the headline and the card is the answer.
    // The `#/` is not cosmetic — this href was written by hand without one, so
    // the browser resolved it against the ORIGIN and left the app entirely.
    // It is `insightsHref()` now, and this is what that helper produces.
    expect(link).toHaveAttribute('href', '#/insights?plan=demo');
    expect(await screen.findByText(/finishes ~/)).toBeTruthy();
  });

  it('says nothing about money when a plan has never cost anything', async () => {
    // A plan with run files that cost nothing — the case a bare `detail.cost`
    // check would render as `$0.00`. Not a fact about spending; a row that
    // should not be there.
    vi.mocked(api.plan).mockResolvedValue({
      ...DETAIL,
      cost: {
        ...
        byDay: [],
        byDayTruncated: false,
        runs: [{ runId: 'run-1', spentUsd: 0, budgetUsd: null }],
      },
    } as unknown as PlanDetail);
    renderPlan(['demo']);
    await screen.findByRole('link', { name: 'P2 ready' });
