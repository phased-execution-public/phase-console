...
# `.invalid` is reserved by RFC 2606 for exactly this: a TLD guaranteed never to
# resolve, so an address under it cannot be anyone's. Tests that need a git
# `user.email` reach for it, and an allowlist entry per test would be a list
# that grows for no reason.
PLACEHOLDER_RE='@(example\.(com|org|invalid)|x\.com)$'
TOOL_WORD='gra'
