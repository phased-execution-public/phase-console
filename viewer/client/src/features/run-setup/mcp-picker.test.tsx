...
// The `Checkbox` primitive is a Radix `<button role="checkbox">`, so the state
// is `aria-checked` rather than `.checked` — the role query is the same.
const box = (id: string) => screen.getByRole('checkbox', { name: new RegExp(id) }) as HTMLButtonElement;
...
describe('the MCP picker', () => {
  ...
  /**
   * A run was started with `sentry` attached; its token expired overnight and
   * the run halted. Continue rendered the row TICKED (it is on the run) and
   * GREYED (it is unusable) — so the run could not be continued without the
   * server, and could not be continued with it either. The only way out was to
   * abandon the run and start a fresh one.
   *
   * A broken server may never be newly ticked. One already ON the run must
   * stay untickable-OFF.
   */
  it('lets a chosen server that has since broken be UNticked', async () => {
    const onChange = vi.fn();
    mount([server({ id: 'sentry', status: 'needs-auth' })], { chosen: ['sentry'], onChange });
    fireOpen();

    const row = box('sentry');
  ...
  it('still refuses to newly tick a broken server that is NOT on the run', async () => {
    const onChange = vi.fn();
    mount([server({ id: 'sentry', status: 'needs-auth' })], { chosen: [], onChange });
    fireOpen();

    expect(box('sentry').disabled).toBe(true);
