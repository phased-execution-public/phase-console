...
export const NOTE_ORDER = [
  ...
  'pause',
  'frozen',
  // Two live branches would not merge. ADVISORY: nothing has failed, nothing
  // has stopped, and the run carries on — which is exactly why it sits here
  // rather than lower. A conflict is cheapest to undo while both lanes are
  // still working, and by the time it is a halt the choice has been made for
  // you. It never wears `error`: the radar is a prediction about a merge that
  // has not happened, and a healthy run's page must not turn red on one.
  'conflict',
  // The run stopped ON a clock and is waiting for a person — worse than
  // `waiting-window` (which resumes itself) and better than a halt. The SLOT is
  // about who must act; `waitReasonOf` says which wait it is, and the two are
  // independent (both waits can end up in either slot).
  'limit-paused',
  'waiting-window',
  'budget',
  'scoped',
...
function budgetRaiseText(run: RunState | null): string | undefined {
  ...
  return `The autopilot already raised the run budget once, from ${money(raise.from)} to ${money(raise.to)} (+${raise.pct}%).`;
}
...
export function runNotes({
   ...
   *
   * Passed in rather than read here, for the same reason `recovery` is: this
   * stays a pure function of what it is given. `null`/`undefined` is the
   * ordinary answer — a shared run, a stopped run — and raises no note, which
   * is the honest reading: an unprobed pair is not a clean one, but it is not
   * a conflict either.
   */
  git?: RunGitView | null;
  onClearScope?: () => void;
  ...
  if (run?.halt) {
    ...
    // `running` and leaves the halt standing as the record of WHY the run
    // stopped, because clearing it up front lost the original reason every time
    // the recovery then crashed — success, and only success, retires it. That
    // contract has exactly one reader and this was it, and it read the halt
    // alone. The measured cost: a QA-fix session three minutes into editing
    // files rendered as `Halted. nothing left to run on its own — phase 8 …`
    // under an error banner offering to repair it, and the operator did what
    // the banner asked and intervened by hand on a run that was already healing
    // itself. Same words, same reason, same phase — demoted to what they now
    // are, the stop this run is presently working its way out of. No remedy
        ...
        <>
          ...
          {run.pause?.requestedAt && (
            <span className="text-ink-faint">
              {' '}
              (asked for <RelativeTime at={run.pause.requestedAt} />)
            </span>
          )}
  ...
  // Two branches the console can see would not merge. Advisory and `warn`,
  // never `error`: the merge has not happened, nothing has failed, and the run
  // is still working — the note exists because serializing two lanes is cheap
  // NOW and expensive once one of them has landed.
  const conflicted = (git?.radar ?? []).filter((pair) => pair.state === 'conflicted');
  if (conflicted.length) {
    const files = [...new Set(conflicted.flatMap((pair) => pair.files))];
    notes.push({
      id: 'conflict',
      severity: 'warn',
      title: `${plural(conflicted.length, 'branch pair')} would not merge.`,
      body: (
        <>
          {conflicted
            .slice(0, 3)
            ...
            .join(', ')}
          {conflicted.length > 3 ? ` +${conflicted.length - 3} more` : ''}
          {files.length > 0 && (
            <span className="mt-1 block text-2xs text-ink-faint">
              over <code className="font-mono">{files.slice(0, 5).join(', ')}</code>
              {files.length > 5 ? ` +${files.length - 5} more` : ''}
            </span>
          )}
          <span className="mt-1 block text-2xs text-ink-faint">
            Nothing has failed and nothing has stopped — this is a merge that has not happened yet. The card
            below has the whole radar.
          </span>
        </>
      ),
    });
  ...
  // Two different facts share `waitUntil`, and telling a person the wrong one
  ...
  if (run?.waitUntil && run.status === 'paused') {
    ...
    notes.push({
      ...
      title:
        waitReason === 'external'
          ? 'Stopped while waiting on external work.'
          : `Stopped on a usage limit${run.accountId ? ` (${run.accountId})` : ''}.`,
      body:
        waitReason === 'external' ? (
          <>
            A phase is parked on work outside its session and is due back at {at(run.waitUntil)}. Nothing is
            spent while it waits and no account is at its limit — Continue picks the phase's own session back
            up, either now or when the wait is over.
          </>
        ) : (
          <>
            The window reopens at {at(run.waitUntil)}. Continue now under another account, switch this run's
            account below, or wait — the checkpointed session resumes either way, keeping the work already
            done.
          </>
        ),
    });
    ...
    notes.push({
      ...
      body:
        waitReason === 'external' ? (
          <>
            Waiting on external work until {at(run.waitUntil)} — a phase declared the wait itself. Nothing is
            wrong, no account is at its limit, and the run resumes by itself.
          </>
        ) : (
          <>
...
export function AuthCard({
  ...
  const signIn = async () => {
    ...
    try {
      ...
      // The one error leg. The dialect this replaces printed `undefined` for
      // anything thrown that is not an `Error` — which a rejected fetch is.
      toastError(error);
    } finally {
      setBusy(false);
    }
      ...
      <CardHeader className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        {auth?.checkedAt && (
          <span className="text-2xs text-ink-faint">
            checked <RelativeTime at={auth.checkedAt} />
          </span>
        )}
      </CardHeader>
      <CardBody>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {tokenAccount ? (
            <Button asChild>
              <a href={settingsHref()}>Paste a fresh setup-token in Settings</a>
            </Button>
          ) : (
