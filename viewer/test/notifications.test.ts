...
test('a qa hold announces once per verdict, and not for a closed plan', async () => {
  ...
  // never reached anybody who was not already looking.
  const { Service } = await import('../server/service.ts');
  const { SKILL_DIR } = await import('../server/config.ts');
  const service = new Service({
    port: 0, host: '127.0.0.1', open: false, allowWrites: false,
  ...
  const sent: Array<{ category: unknown; title: string }> = [];
  service.push.announce = ((category: unknown, message: { title: string }) => {
    sent.push({ category, title: message.title });
  }) as typeof service.push.announce;

  const announceQaHold = (service as unknown as {
    announceQaHold: (data: unknown) => void;
  }).announceQaHold.bind(service);

  announceQaHold({ slug: 'demo', runId: 'r1', phase: 2, qaVerdict: 'pending' });
  assert.equal(sent.length, 1);
  assert.equal(sent[0].category, 'qa');
  assert.match(sent[0].title, /phase 2 — QA verdict owed/);

  announceQaHold({ slug: 'demo', runId: 'r1', phase: 2, qaVerdict: 'pending' });
  assert.equal(sent.length, 1, 'the same hold twice is one push');

  announceQaHold({ slug: 'demo', runId: 'r1', phase: 2, qaVerdict: 'pass' });
  assert.equal(sent.length, 1, 'a recorded verdict is not a hold');

  (service as unknown as { isClosedPlan: (slug: string) => boolean }).isClosedPlan = () => true;
  announceQaHold({ slug: 'demo', runId: 'r1', phase: 3, qaVerdict: 'pending' });
  assert.equal(sent.length, 1, 'a closed plan claims nothing about progress');
});
...
test('a notification that reached nobody becomes an environment issue, once (D23)', async () => {
  const { Service } = await import('../server/service.ts');
  const { SKILL_DIR } = await import('../server/config.ts');
  ...
  const before = service.state().environment.issues.length;
  // The aggregate itself is proved in `push.test.ts`; this is the WIRING —
  // that the verdict lands on the health card rather than being pushed down
  // the channel under suspicion, and that it lands once.
  assert.ok(service.push.onUndelivered, 'the service must be listening for it');
  service.push.onUndelivered!({
    category: 'stalled', tag: 'run:abc:stalled', title: 'A lane has gone quiet',
    devices: [
      { label: 'Mac · Chrome', outcome: 'failed', detail: 'BadWebPushTopic' },
      { label: 'iPhone · Safari', outcome: 'failed', detail: 'BadWebPushTopic' },
    ],
  });
  const issues = service.state().environment.issues;
  assert.equal(issues.length, before + 1);
  const minted = issues[issues.length - 1];
  assert.equal(minted.kind, 'push-broken');
  assert.match(minted.detail, /reached none of this console's 2 devices/);
  assert.match(minted.detail, /A lane has gone quiet/);

  service.push.onUndelivered!({
    category: 'halted', tag: 'run:def:halted', title: 'Another one',
    devices: [{ label: 'Mac · Chrome', outcome: 'failed' }],
  });
  assert.equal(service.state().environment.issues.length, before + 1,
    'the operator learns the channel is not arriving — not forty times');

  service.close();
});

/* ------------------------------------------------------------------ *
 * Stalls that escalate, and stall cards that stand themselves down (D22)
 * ------------------------------------------------------------------ */

/**
 * A service with one live run whose phase 3 is stalled, and a stall
 * escalation clock short enough for a test to wait out.
 *
 * The runner is a stub because the escalation's guard reads exactly one thing
 * from it — is this run still live, and is this phase still stalled — and a
 * real runner would have to be driven through a real session to say so.
 */
async function stalledService(escalateMs: number, runId = 'run-d22') {
  const { Service } = await import('../server/service.ts');
  const { SKILL_DIR } = await import('../server/config.ts');
  ...
  inner.prefs.stallEscalateMs = escalateMs;
  ...
  const pushed: { title: string; urgent: unknown }[] = [];
  service.push.announce = ((_c: unknown, message: { title: string }, _n: unknown, _d: unknown,
    opts?: { urgent?: boolean }) => {
    pushed.push({ title: message.title, urgent: opts?.urgent });
  }) as typeof service.push.announce;

  const stall = (signal = 'silent') => inner.onRunnerEvent('run:liveness', {
    slug: 'demo', runId, phase: 3, attempt: 0,
    stall: { signal, detail: 'nothing for ten minutes' },
  });
  const clear = () => inner.onRunnerEvent('run:liveness', {
    slug: 'demo', runId, phase: 3, attempt: 0, stall: null,
  });
  // Scoped to THIS fixture's run: the notification store is persisted and
  // every `new Service()` in this process reads the same one, so an unscoped
  // count is a count of every stall case that ran before.
  const cards = () => service.inbox({ limit: 500 }).items
    .filter((r) => r.category === 'stalled' && r.runId === runId);

  return { service, run, stall, clear, cards, pushed };
}

test('a stall nobody resolved is said once more, urgently (D22)', async () => {
  const { service, stall, cards, pushed } = await stalledService(30);

  stall();
  const first = cards();
  assert.equal(first.length, 1, 'the transition announces once');
  assert.equal(pushed.length, 1);
  assert.equal(pushed[0].urgent, undefined,
    'the first card stays quiet — nothing is blocked on the operator at minute ten');

  for (let i = 0; i < 60 && cards().length < 2; i++) {
    await new Promise((r) => { setTimeout(r, 10); });
  }
  const after = cards();
  assert.equal(after.length, 2, 'the stall outlived the escalation threshold and was said again');
  assert.match(after[0].title, /^Still stalled after \d+ min/);
  assert.equal(pushed.length, 2);
  assert.equal(pushed[1].urgent, true, 'the second one is the urgent one — that is the whole point');

  // ONE re-announcement. A loop here is the thing the non-urgent policy exists
  // to prevent, so nothing more may arrive however long it stays stalled.
  await new Promise((r) => { setTimeout(r, 120); });
  assert.equal(cards().length, 2, 'escalation is once, never a loop');
  service.close();
});

test('a stall that clears stands its own card down (D22)', async () => {
  const { service, stall, clear, cards } = await stalledService(60_000, 'run-d22-clear');

  stall();
  assert.equal(cards().filter((r) => !r.resolved).length, 1);

  clear();
  const open = cards().filter((r) => !r.resolved);
  assert.equal(open.length, 0,
    'every stalled card resolves when its silence ends — 26 raised and 0 resolved was the defect');
  assert.match(cards()[0].resolved?.reason ?? '', /producing work again/);
  service.close();
});

test('a stall card does not outlive its phase or its run (D22)', async () => {
  const ended = await stalledService(60_000, 'run-d22-phase-end');
  const endedInner = ended.service as unknown as { onRunnerEvent: (e: string, d: unknown) => void };
  ended.stall();
  endedInner.onRunnerEvent('run:phase', {
    slug: 'demo', runId: 'run-d22-phase-end', phase: 3, status: 'done',
  });
  assert.equal(ended.cards().filter((r) => !r.resolved).length, 0,
    'a lane that stalled and then finished emits no liveness clear — the phase end is the trigger');
  ended.service.close();

  const settled = await stalledService(60_000, 'run-d22-settled');
  const settledInner = settled.service as unknown as { onRunnerEvent: (e: string, d: unknown) => void };
  settled.stall();
  settled.run.status = 'halted';
  settledInner.onRunnerEvent('run:run', { state: settled.run });
  assert.equal(settled.cards().filter((r) => !r.resolved).length, 0,
    'whatever the lane was doing when it went quiet, a settled run is not doing it now');
  settled.service.close();
});

test('a recycled session stands its own stall card down, and says what was done (P20)', async () => {
  // The watchdog ends the session a stall card is ABOUT, and no liveness clear
  // follows — the ticker stops with the lane. Without this the card would
  // outlive its subject, the same defect D22 fixed from the other end. QA round
  // 1: `announceWatchdog` shipped with no test at all.
  const { service, stall, cards } = await stalledService(60_000, 'run-p20-recycle');
  const inner = service as unknown as { onRunnerEvent: (e: string, d: unknown) => void };

  stall();
  assert.equal(cards().filter((r) => !r.resolved).length, 1);

  // A nudge is deliberately silent here: one line written to a session that may
  // answer in seconds, and it is in the journal.
  inner.onRunnerEvent('run:watchdog', {
    slug: 'demo', runId: 'run-p20-recycle', phase: 3, action: 'nudged', attempt: 0, nudges: 1,
  });
  assert.equal(cards().filter((r) => !r.resolved).length, 1, 'a nudge neither announces nor retracts');

  inner.onRunnerEvent('run:watchdog', {
    slug: 'demo', runId: 'run-p20-recycle', phase: 3, action: 'recycled',
    attempt: 0, recycles: 1, sessionId: 'sess-abc',
  });
  const stallCards = cards().filter((r) => !/silent session recycled/.test(r.title));
  assert.equal(
    stallCards.filter((r) => !r.resolved).length, 0,
    'the card about the session came down with the session',
  );
  assert.match(stallCards[0].resolved?.reason ?? '', /recycled the silent session/);
  const said = cards().find((r) => /silent session recycled/.test(r.title));
  assert.ok(said, 'and the operator is told what the console did on their behalf');
  assert.match(said!.body ?? '', /same session id/, 'a resumed session says so');
  service.close();
});

test('a recycle with NO session id does not claim the session was resumed (P20)', async () => {
  // A sentence is a claim. `checkpointLane` makes exactly this distinction in
  // its own note, and the push body must not be the one place it is lost.
  const { service, stall, cards } = await stalledService(60_000, 'run-p20-no-session');
  const inner = service as unknown as { onRunnerEvent: (e: string, d: unknown) => void };

  stall();
  inner.onRunnerEvent('run:watchdog', {
    slug: 'demo', runId: 'run-p20-no-session', phase: 3, action: 'recycled',
    attempt: 0, recycles: 1, sessionId: null,
  });
  const said = cards().find((r) => /silent session recycled/.test(r.title));
  assert.ok(said);
  assert.ok(!/same session id/.test(said!.body ?? ''), 'it never had one to resume');
  assert.match(said!.body ?? '', /starts again from its boot prompt/);
  service.close();
});

test('an escalation is disarmed by a clear, so a lane that got going is never buzzed (D22)', async () => {
  const { service, stall, clear, cards, pushed } = await stalledService(40, 'run-d22-disarm');

  stall();
  clear();
  await new Promise((r) => { setTimeout(r, 150); });
  assert.equal(cards().length, 1, 'the cleared stall must not escalate after the fact');
  assert.equal(pushed.filter((p) => p.urgent === true).length, 0);
  service.close();
});

test('a zero escalation clock is the off switch, not an instant one (D22)', async () => {
  // The shape that would have been the exact inverse of the setting: an
  // operator asking for the pre-escalation behaviour back and getting an
  // urgent card on the very next tick. `sanitiseAutomation` keeps the zero
  // (this is the one stall number that takes `cap`), so the guard has to be
  // in the arming.
  const { service, stall, cards } = await stalledService(0, 'run-d22-off');
  stall();
  await new Promise((r) => { setTimeout(r, 120); });
  assert.equal(cards().length, 1, 'one quiet card per episode, exactly as before the escalation existed');
  service.close();
});

/* ------------------------------------------------------------------ *
 * The urgency override (parallel-repaint P2)
 * ------------------------------------------------------------------ */

test('a per-announcement urgency override is written to the record', () => {
  // The escalated stall is announced URGENT while its category stays quiet.
  // The record used to copy the catalogue and read "not urgent" about the one
  // push that buzzed a wrist — the ledger disagreeing with the wire.
  const inbox = store();
  const plain = inbox.record(announcement({ category: 'stalled' }));
  assert.equal(plain.urgent, false, 'the catalogue decides when nothing overrides it');
  const escalated = inbox.record(announcement({ category: 'stalled', urgent: true }));
