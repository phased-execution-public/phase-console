 ...
 *
 * A setup prompt appears on the Settings page, in the in-app guide and in the
 * README. Two of those are markdown files a person edits by hand, so without a
 * check the copies drift — and an install procedure that is subtly wrong in one
 * place is worse than one that is missing, because the reader has no reason to
 * doubt it.
 *
...
import {
  SETUP_PROMPTS, INSTALL_PROMPT, TAILSCALE_PROMPT,
