...
/**
 * Inline markdown is parsed once per distinct string, and sharing the parsed
 * fragment must not let the first render blank the second.
 *
 * That second property is the one worth a test rather than a look: a
 * `DocumentFragment` is EMPTIED by the `replaceChildren` that consumes it, so a
 * cache that handed out the original instead of a clone would render row one
 * correctly and leave every later row with the same title blank — on a board,
 * intermittently, depending on which rows the window happened to mount.
 *
 * Every fixture uses a string of its own, because the cache is module-scoped
 * and outlives a test.
 */
describe('the inline markdown cache', () => {
  it('parses a given string once however many times it is rendered', () => {
    const parse = vi.spyOn(marked, 'parseInline');
    const text = 'a **cached** title';

    const first = render(<MarkdownInline text={text} />);
    const calls = parse.mock.calls.length;
    expect(calls).toBeGreaterThan(0);
    first.unmount();

    // A fresh mount — a row scrolled back into the window — and two more
    // beside it. None of them may reach the parser again.
    render(
      <div>
  ...
  it('renders every copy, so a shared fragment is never spent by the first', () => {
    const { container } = render(
      <div>
        <MarkdownInline text="row **one** and the same" />
        <MarkdownInline text="row **one** and the same" />
        <MarkdownInline text="row **one** and the same" />
      </div>,
    );
    const spans = container.querySelectorAll('.md-inline');
    expect(spans).toHaveLength(3);
    for (const span of spans) {
      expect(span.querySelector('strong')?.textContent).toBe('one');
      expect(span.textContent).toBe('row one and the same');
    }
  });

  it('is bounded — a long-lived tab cannot grow it without limit', () => {
    const before = inlineCacheSize();
    for (let i = 0; i < 400; i++) {
      const { unmount } = render(<MarkdownInline text={`bounded fixture ${i}`} />);
      unmount();
    }
    const after = inlineCacheSize();
    expect(after).toBeGreaterThan(before);
    // The cap is 256; the assertion is on the bound existing, not its value.
    expect(after).toBeLessThanOrEqual(256);
  });
});
