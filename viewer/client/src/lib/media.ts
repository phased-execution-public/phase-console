...
/**
 * "Do not animate" — asked here because CSS alone cannot answer it.
 *
 * `theme.css` carries the blanket `@media (prefers-reduced-motion: reduce)` rule
 * that flattens every animation, transition and CSS `scroll-behavior`. It cannot
 * reach a scroll the app performs in JS: per CSSOM-View an explicit
 * `behavior: 'smooth'` on `scrollTo` OVERRIDES the computed `scroll-behavior`,
 * so the one place the app scrolls a pane itself sailed straight past the
 * preference. Any imperative animation must consult this instead of assuming
 * the stylesheet has it covered.
 */
...
/*
 * There is deliberately no `useReducedMotion` hook beside the other three.
 * Nothing here RENDERS differently under the preference — the stylesheet
 * handles every case that paints — so a hook would be an unused export whose
 * only effect is to invite a component to re-render on a preference change it
 * does not read. Add one when a component genuinely needs to branch.
 */

/** The scroll behaviour to hand a DOM scroll call — `auto` is "jump, do not animate". */
