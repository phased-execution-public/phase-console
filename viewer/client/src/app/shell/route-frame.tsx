...
export function RouteFrame({
  ...
  // The boundary is OUTSIDE the Suspense boundary on purpose: a lazy chunk that
  // fails to load throws from the Suspense boundary itself, and a boundary
  // nested inside it would never see that. `resetKey` is the path, so
  // navigating away from a broken destination and back gives it a fresh try.
  const body = (
    <ErrorBoundary label={routeAnnouncement(route)} resetKey={route.path}>
      <Suspense
        fallback={
