 ...
 * and then immediately ask it to subscribe. Registering it for everyone
 * separates the two: the worker is how the app loads, and push is a thing the
 * worker can additionally do.
 *
 * **Unconditional, but not at boot.** The shell registers it through
 * `afterLoadIdle` — the precache is 1.5 MB fetched with `cache: 'reload'`, and
 * paying for the second visit's speed out of the first visit's first paint is
 * the wrong way round. `lib/push.ts` still calls `registerServiceWorker()`
 * directly, because by then a person has pressed a button and the wait IS the
 * work.
 *
...
/**
 * Run something once the page has loaded and the main thread is idle.
 *
 * Registering the worker at boot is not free. `navigator.serviceWorker.register`
 * fetches the script and then the worker's `install` handler fetches the whole
 * precache — 1,546,440 B over 51 entries with `cache: 'reload'`, which is a
 * deliberate bypass of the HTTP cache — and all of it competes with the very
 * first plan open for the same connections. The precache is for the SECOND
 * visit; paying for it during the first paint of the first visit is the exact
 * inversion of what it is for.
 *
 * `requestIdleCallback` with a timeout is the right instrument: the browser
 * picks a moment nothing is on the critical path, and the timeout means a busy
 * page still gets its worker rather than never.
 *
 * Returns a canceller, because an unmounting shell must not leave a callback
 * that registers a worker for a page that is gone.
 */
  ...
  const idle = () => {
    ...
    if (typeof ric === 'function') {
      const id = ric(
        ...
        // Safari has no `requestIdleCallback` at all (hence the fallback), and
        // a page that never goes idle must not simply never install a worker.
        { timeout: 3_000 },
      );
      release = () =>
        (globalThis as { cancelIdleCallback?: typeof cancelIdleCallback }).cancelIdleCallback?.(id);
      return;
    }
    const id = setTimeout(() => {
      if (!cancelled) run();
    }, 0);
    release = () => clearTimeout(id);
  };

  if (typeof document !== 'undefined' && document.readyState !== 'complete') {
    const onLoad = () => idle();
    window.addEventListener('load', onLoad, { once: true });
    release = () => window.removeEventListener('load', onLoad);
  } else {
    idle();
  }
...
export function useServiceWorker(): void {
  useEffect(() => {
    ...
    // AFTER first paint, never during it — see `afterLoadIdle`. The worker is
    // what makes the NEXT visit fast; fetching its 1.5 MB precache alongside
    // the first plan open makes this one slower to buy that.
    const stopWaiting = afterLoadIdle(() => {
      void registerServiceWorker().then((reg) => {
        if (!reg || cancelled) return;
        registration = reg;

        // `controller` is what distinguishes an update from a first install: on
        // a first install there is nothing to interrupt, so the worker simply
        // takes over and nobody is told. A boot-time waiting worker is the
        // reported failure exactly — the cached shell just rendered — so it is
        // applied, not offered, unless a guard says otherwise.
        ...
        // One check per load. The browser does this on its own schedule too,
        // but its schedule can be a day, and "I just deployed and it is still
        // old" is exactly the confusion this is here to prevent.
