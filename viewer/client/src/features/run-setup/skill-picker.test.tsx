...
describe('SkillPicker default badges', () => {
  ...
  /**
   * A QA review seeds `chosen` from the plan's own skills, so both props got
   * the SAME array: the summary read "2 chosen · 2 from the plan" and the same
   * two ids appeared twice — once as immutable plan chips carrying "edit the
   * plan if they are wrong", and immediately below as removable ones. Removing
   * one did nothing an operator could see, because the plan puts it back into
   * every boot prompt regardless.
   */
  it('does not list a plan skill a second time as removable', () => {
    mount({ chosen: ['graph-tool', 'browse'], planSkills: ['graph-tool', 'browse'] });
    // The plan's chips are there…
    expect(screen.getByText('2 from the plan')).toBeTruthy();
    // …and nothing claims to be separately chosen, nor offers to remove them.
    expect(screen.queryByText(/\d+ chosen/)).toBeNull();
    expect(screen.queryByRole('button', { name: 'Remove graph-tool' })).toBeNull();
    expect(screen.queryByRole('button', { name: 'Remove browse' })).toBeNull();
  });

  it('still lists a chosen skill the plan does NOT name', () => {
    mount({ chosen: ['graph-tool', 'browse'], planSkills: ['graph-tool'] });
    expect(screen.getByText('1 chosen')).toBeTruthy();
    expect(screen.getByText('1 from the plan')).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Remove browse' })).toBeTruthy();
    expect(screen.queryByRole('button', { name: 'Remove graph-tool' })).toBeNull();
  });
});
