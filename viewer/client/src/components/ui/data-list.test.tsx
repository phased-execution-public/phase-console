...
describe('DataList', () => {
  ...
  /**
   * ARIA requires a `listitem` to be OWNED by a `list`, and a virtualiser has
   * to put a sized, positioned spacer between the two. Without `presentation`
   * on that wrapper the rows' role is ignored — so "item 3 of 40", the whole
   * reason `aria-setsize`/`aria-posinset` are set on each row, reaches nobody.
   */
  it('keeps list → listitem ownership through the virtualiser spacer', () => {
    const { container } = render(
    ...
    // jsdom has no layout, so a virtualiser can legitimately render zero rows
    // — and a loop over zero rows would pass this test without testing it.
    const rows = list.querySelectorAll('[role="listitem"]');
    expect(rows.length).toBeGreaterThan(0);
    for (const row of list.querySelectorAll('[role="listitem"]')) {
      // Every element between the row and the list must be transparent to ARIA.
      for (let node = row.parentElement; node && node !== list; node = node.parentElement) {
        expect(['presentation', 'none']).toContain(node.getAttribute('role'));
      }
    }
  });
});
