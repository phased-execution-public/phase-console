...
import { toastError, useConsoleState, useDirListing, useRootCheck } from '@/lib/queries';
import {
  Banner,
  Button,
  Card,
        ...
        <CardBody>
          <label className="block">
            ...
            <input
              ...
              className={cn(
                field,
                // The border tint is the pointer affordance; the ring is the
                // keyboard one. This carried `focus:outline-none` as well,
                // which took the global `:focus-visible` ring away from BOTH —
                // leaving a 1px hue change as the only indication of focus, on
                // the one input that decides which repository the console
                // opens. `:focus-visible` paints on keyboard focus only, so
                // keeping it costs the pointer case nothing.
                'w-full py-2 font-mono placeholder:text-ink-faint focus:border-action',
              )}
            />
          </label>
        </CardBody>
