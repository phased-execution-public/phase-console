...
import { CLOSED_PLAN_STATUSES, HANDOFF_STATUSES, QA_RESULTS } from '@shared/plan-vocab.js';
...
export const FORMS: Record<string, Form> = {
  ...
  'qa-record': {
    ...
    fields: [
      { name: 'result', label: 'Result', type: 'select', options: [...QA_RESULTS] },
      {
        name: 'report',
  ...
  'close-plan': {
    ...
    fields: [
      { name: 'status', label: 'Close as', type: 'select', options: [...CLOSED_PLAN_STATUSES] },
      {
        name: 'reason',
        label: 'Why (one line)',
