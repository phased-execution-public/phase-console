...
import { Button, Dialog, DialogClose, DialogContent, type ButtonProps } from '@/components/ui';
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <RunSetup
            ...
            blocked={!brief.trim() || !rootOpen || start.isPending}
            blockedReason={
              start.isPending
                ? 'Starting the session…'
                : rootOpen
                  ? 'Write the brief first — it is what the session is given.'
                  : undefined
            }
            onLaunch={(body: Record<string, unknown>) => start.mutate(body)}
            cancel={
              <DialogClose asChild>
