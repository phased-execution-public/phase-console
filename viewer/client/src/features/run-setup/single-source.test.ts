...
describe('RunSetup is the only launch form', () => {
  ...
  it('every settings patch outside the form is a ONE-TAP verb, named field by field', () => {
    ...
    const patches = (path: string): string[] => {
      const file = FILES.find((candidate) => candidate.path === path)!;
      return [...file.text.matchAll(/api\.runSettings\([^,]+, (\{[^}]*\})\)/g)].map((match) =>
        match[1]!.replace(/\s+/g, ' ').trim(),
      );
    };

    expect(patches('features/runs/run-page.tsx')).toEqual([
      '{ onlyPhases: [] }',
  ...
  it('nothing outside the one session helper mints an agent ticket', () => {
    // Four callers, and what they have in common is that each owns a PTY,
    // not a form: the shared session helper (QA + recovery), the sessions
    // page's own terminal, the plan wizard — whose ticket carries an `intent`
    // and a `brief` no run door has — and the issues board's launch, which is
    // that same wizard ticket plus the one thing a wizard cannot know, WHICH
    // issues. All four take their FIELDS from RunSetup (`mode="plan"` for the
    // last two, the same field set and the same `buildLaunch`); none of them
    // declares one. A caller that declared its own fields would be a fifth
    // form, and that is what this list is against — not the count.
    expect(
      offenders(/\bapi\.agentTicket\b/, [
        'lib/start-session.ts',
        'features/sessions/session-page.tsx',
        'features/sessions/wizard.tsx',
        'features/repo/issues-launch.tsx',
      ]),
    ).toEqual([]);
  });
