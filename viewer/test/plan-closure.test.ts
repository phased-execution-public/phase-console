...
test('a closed plan reports no progress problems, and an open one still does', async (t) => {
  ...
  // WARNING, not error, since 2026-08-30: a `blocked` handoff is a session doing
  // exactly what the contract asks, and an ERROR here is what the situation
  // classifier's health arm read as "the plan is broken" — so the session's own
  // testimony was outranked by an issue its honesty raised.
  assert.ok(open.issues.some((i) => i.kind === 'stale-handoff' && i.severity === 'warning'),
    'an open plan with a blocked handoff is worth a look');
  ...
  assert.equal(open.issueCounts.warning > 0, true);
