...
import { Card, CardBody, CardHeader, CardTitle, Empty, Legend } from '@/components/ui';
...
/**
 * The three ways a phase's wall clock goes, and the ONE class that colours each.
 *
 * Every one of them is a `.state-*` class setting `--state`, which is the same
 * bridge the Gantt's bars paint through (`gantt.tsx` `BAR_STYLE`) and the same
 * one every badge in the console uses. They disagreed: `waiting` was
 * `bg-accent` — which is `--status-needs-you` — so the same word was drawn in
 * two different hues on two cards of the same page, one of them the colour
 * reserved for "somebody has to do something".
 *
 * `frozen` is deliberately not a status hue: nothing is wrong and nothing is
 * moving.
 */
const SEGMENTS = [
  { key: 'workingMs', label: 'working', className: 'state-running bg-state' },
  { key: 'waitingMs', label: 'waiting', className: 'state-waiting bg-state' },
  { key: 'frozenMs', label: 'frozen', className: '[--state:var(--ink-faint)] bg-state/50' },
] as const;
...
export function Timeline({
  ...
  now?: number;
  /** Where an empty card sends you — the page knows the plan, this does not. */
      ...
      <CardBody>
        {spans.length ? (
          <>
                ...
                <li key={span.phase} className="grid grid-cols-[3rem_1fr_auto] items-center gap-2">
                  ...
                  <span
                    ...
                    aria-label={`phase ${span.phase}: ${SEGMENTS.filter((s) => span[s.key] > 0)
                      ...
                      .join(', ')}`}
                    className="flex h-2.5 overflow-hidden rounded-full bg-ground-deep"
            ...
            {/* The shared key, drawing each band's own paint. */}
            <Legend
