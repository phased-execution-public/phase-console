...
import { laneOf, parseHash, phaseSessionHref, type Route } from './routes';
...
describe('?lane=pN — the third query reader, beside focus and panel', () => {
  // B2(c). A lane was `useState` in the tab strip, so the pane an operator was
  // pointed at from a phase chip could not be linked to at all. It is a HINT,
  // not an address: it decides which tab opens, and the strip still owns the
  // choice.
  it('reads the phase number a lane tab is spelled with', () => {
    expect(laneOf(route('#/plan/demo/run?lane=p7'))).toBe(7);
    expect(laneOf(route('#/plan/demo/run?lane=p0'))).toBe(0);
    expect(laneOf(route('#/plan/demo/run?lane=p123'))).toBe(123);
  });

  it('ignores anything it cannot read, exactly as an unknown `focus` is ignored', () => {
    // A URL is user input. A bad one must not select an arbitrary lane.
    for (const hash of [
      '#/plan/demo/run',
      '#/plan/demo/run?lane=',
      '#/plan/demo/run?lane=7', // the `p` is the spelling, not decoration
      '#/plan/demo/run?lane=phase-7',
      '#/plan/demo/run?lane=p-1',
      '#/plan/demo/run?lane=p99999', // wider than any plan; not a lane id
      '#/plan/demo/run?lane=p7x',
    ]) {
      expect(laneOf(route(hash)), hash).toBeUndefined();
    }
  });

  it('round-trips what `phaseSessionHref` builds', () => {
    // The two ends of the same link: the server says a phase is live, the href
    // builder points at its lane, and this reads the lane back out.
    const href = phaseSessionHref({ slug: 'demo', phase: 9, live: { via: 'run' } })!;
