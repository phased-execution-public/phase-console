 ...
 */
function mcpDep(configFiles: string[][] = [], configPaths: string[] = []) {
  return {
    ...
    configFor: async (runId: string, phase: number, ids: string[]) => {
      configFiles.push([...ids]);
      // Per run AND phase, so two lanes cannot land on one path. Recorded so
      // the concurrency test below can assert the paths actually differ.
      // The real basename scheme (`mcpConfigName`), spelled out rather than
      // imported: `mcp/config.ts` resolves INSTANCE_STATE_DIR at module load,
      // and this suite deliberately does not load the state sandbox. The name
      // itself is pinned against `writeMcpConfigFile` in `mcp.test.ts`.
      configPaths.push(`/tmp/run-${runId}-p${phase}.json`);
      return ids.length ? configPaths[configPaths.length - 1] : null;
    },
  };
}
...
async function drive(r: Repo, options: DriveOptions = {}) {
  ...
  const runner = new Runner({
    ...
    mcp: mcpDep(configFiles, configPaths),
  ...
  return { prompts, requests, configFiles, configPaths, degraded, state, ran: prompts.length > 0 };
}
...
test('exit criterion 1: each lane gets its OWN --mcp-config, always paired with --strict-mcp-config', () => {
  // Two halves of one guarantee, and they are asserted at the two layers that
  // own them.
  //
  // The PATH is per (run, phase) — `configFor` is handed the phase now. The
  // document was always built per phase (a phase's servers are the union of the
  // plan's, the run's and its own), but the file was keyed by run alone, so a
  // run driving two lanes at once had both writing one path: whichever boarded
  // second overwrote the first. Under `--strict-mcp-config` that file is the
  // ONLY set the session gets, so a phase could spawn holding another phase's
  // servers, or none of its own.
  const paths: string[] = [];
  const dep = mcpDep([], paths);
  void dep.configFor('run1', 3, ['ctx7']);
  void dep.configFor('run1', 4, ['gh']);
  assert.equal(new Set(paths).size, 2, 'two lanes of ONE run, two paths');
  assert.match(paths[0], /run-run1-p3\.json$/);
  assert.match(paths[1], /run-run1-p4\.json$/);

  // And the PAIRING, at the one place that assembles argv. `--mcp-config`
  // alone would UNION the machine's own servers into an unattended run, so the
  // two flags are pushed together and cannot come apart; a phase degraded to
  // zero reachable servers still passes `strictMcp`, because an emptied set
  // must stay a CLOSED one.
  const spawnSrc = readFileSync(
    new URL('../server/runner/spawn.ts', import.meta.url), 'utf8',
  );
  assert.match(
    spawnSrc,
    /argv\.push\('--mcp-config', request\.mcpConfig, '--strict-mcp-config'\)/,
    'the two flags are one push — see spawn.ts',
  );
  assert.match(spawnSrc, /else if \(request\.strictMcp\) argv\.push\('--strict-mcp-config'\)/,
    'and an empty set stays closed');
});
