...
/**
 * A pulse lane's facts, in the shared bag.
 *
 * `PulseLane` is built from `RunState.children` rather than from `nowLanes()`,
 * so it is its own shape and always will be — which is exactly why the shared
 * primitive takes a bag rather than a model. Four fields, one adapter, and the
 * card stops formatting money and elapsed for itself.
 *
 * `costUsd: … || null` and not `?? null`: this card has always HIDDEN a zero
 * (a lane that has spent nothing is not worth a line here), where the runs
 * strip shows `$0.00`. Absent means "do not draw it" in the bag, so the
 * difference stays a caller's decision instead of becoming a second rule.
 */
function pulseFacts(lane: PulseLane): LaneFacts {
      ...
      <div className="flex flex-col gap-1 p-2">
        {lanes.map((lane) => (
          ...
          >
            ...
            {/* Two facts, drawn by the shared atoms rather than re-picked
                here — `components/lane-facts.tsx`. The COLUMN is this card's
                (stacked, right-aligned, with the clock glyph); the formatting,
                the missing-value hedge and the tick are everybody's. */}
            <span className="shrink-0 text-right">
              <span className="block text-sm text-ink" aria-label="running for">
                <Clock3 size={12} className="mr-1 inline-block align-[-1px] text-ink-faint" aria-hidden />
                <LaneElapsed facts={pulseFacts(lane)} live={!lane.frozen} className="text-sm text-ink" />
              </span>
              <LaneCost facts={pulseFacts(lane)} className="mt-0.5 border-0 bg-transparent px-0 py-0" />
            </span>
          </button>
        ))}
