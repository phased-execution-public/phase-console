...
import type { HealthIssue, Portfolio, Sizing } from '@/lib/api';
...
export function PortfolioPanel({
  ...
  /** The engine's own `sizing.env`, as the server read it. See the caption below. */
  sizing?: Sizing;
}) {
      ...
      <div className="grid min-w-0 gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardBody>
            <BarList
              label="statuses"
              items={stats.byStatus.map((row) => ({ name: row.status, value: row.count }))}
            />
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <CardBody>
            ...
            />
            {/* The caption asserts its own provenance — "from the skill's
                sizing.env" — which makes it a CLAIM rather than a label, and
                the three numbers were typed in here. `sizing.env` invites
                re-rating them, so the caption would go on citing a file it no
                longer agreed with. `settings/essentials.tsx` already reads them
                off state; this now does too. */}
            <p className="mt-2 text-2xs text-ink-faint">
              S {weight(sizing?.S)} · M {weight(sizing?.M)} · L {weight(sizing?.L)} working-set weight, from
              the skill&rsquo;s sizing.env
            </p>
          </CardBody>
        </Card>
      </div>
                ...
                <ul className="flex flex-col gap-1">
                  {stats.activeLocks.map((lock) => (
                    <li key={`${lock.slug}-${lock.phase}`} className="flex items-center gap-2">
                      {/* `tap-row`, not `tap-line`: these are 20px rows at a
                          24px pitch, and an overlay 44px tall reaches 12px into
                          the row above — where, being positioned, it wins. Each
                          row's floor is its own box. */}
                      <a
                        href={phaseHref(lock.slug, lock.phase)}
                        className="tap-row shrink-0 font-mono text-2xs text-ink hover:text-action"
                      >
                        {lock.slug} P{lock.phase}
                      </a>
                      {/* Truncated, with the whole owner on hover: a lock
                          owner is `pe-<host>-<pid>-<epoch>` and the half that
                          identifies the machine is the half a narrow column
                          cuts off. The row cannot say WHICH SESSION holds it —
                          `/api/stats` sends the owner string and no session id
                          — so the owner is the whole answer this card has. */}
                      <span
        ...
        <CardBody>
          {issues.length ? (
            ...
            <Empty
              ...
              /* A filtered empty list and a genuinely clean estate are two
                 different answers, so they get two different ways out: one
                 widens the filter that is hiding the rest, the other goes to
                 the thing you would look at next. */
              action={
                severity === 'all' ? (
                  <Button asChild size="sm">
                    <a href={plansHref()}>Browse the plans</a>
                  </Button>
                ) : (
                  <Button size="sm" onClick={() => setSeverity('all')}>
                    Show every issue
                  </Button>
                )
              }
            ...
            <ul className="flex flex-col gap-1">
              {stats.stalled.map((item) => (
                <li key={item.slug} className="flex flex-wrap items-baseline justify-between gap-2">
                  {/* The box, not an overlay — see the Locks list above. */}
                  <a
                    href={planHref(item.slug)}
                    className="tap-row w-fit font-mono text-2xs text-ink hover:text-action"
                  >
                    {item.slug}
                  </a>
                  <span className="text-2xs text-ink-faint">
