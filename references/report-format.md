...
One report per QA **round**, from `assets/report-template.md`: round 1 is
`docs/handoffs/<slug>/reports/phase-NN-qa.md` and round R > 1 is
`docs/handoffs/<slug>/reports/phase-NN-qa-roundR.md`. **Never choose the name yourself** — the brief from
`phase-graph.sh <slug> --qa-prompt <N>` names the file to write, which is what stops a second reviewer
overwriting the first's report (it happened: 22 report files across 12 phases, one round each).
It must let a reader (human or a future session) trust the verdict without
