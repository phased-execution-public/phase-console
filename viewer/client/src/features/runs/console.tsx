...
import { Button, Input, copy } from '@/components/ui';
...
export function useLiveLines() {
  ...
  /**
   * Seed the task panel from the RUN RECORD, once.
   *
   * The replay above cannot be trusted for this and never could: the transcript
   * is read as a 400-entry tail, and in a measured 10,792-entry run not one of
   * its 85 `TaskCreate`s fell inside that window — so the panel folded a stream
   * of updates to rows it had never seen and stayed empty. The server's fold
   * (`PhaseRecord.tasks`) sees every line, so it is the complete list; this is
   * the shorter path to the same answer.
   *
   * Applied only when the record has MORE rows than the panel. The two folds
   * are the same function over the same events, so the record can only be a
   * superset of a bounded replay — and once live events have carried the panel
   * further, the panel is the newer of the two and keeps its place.
   *
   * **`todosAt` is deliberately left alone.** It used to be stamped `Date.now()`
   * here, which made the card's "Written at" read the moment the PAGE was
   * opened — on precisely the long runs this path exists for, since the record
   * is the authoritative source exactly when the replay window has missed the
   * creates. The record carries the rows and not the moment they were written,
   * so the honest answer is to say nothing: `writtenAt()` renders nothing for
   * `0`. A replay that did see the writes has already stamped the real time,
   * and this keeps it.
   */
  const seedTasks = useCallback((tasks: readonly Todo[] | undefined) => {
    if (seeded.current || !tasks?.length) return;
    seeded.current = true;
    setPanels((current) =>
      tasks.length > current.todos.length ? { ...current, todos: [...tasks] } : current,
    );
  }, []);
  ...
  return { lines, activity: panels, record, clear, hydrate, seedTasks };
}
  ...
  return (
    <section className="rounded-lg border border-rule bg-surface-raised">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-rule px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          {/* Both take a floor and permission to shrink. `truncate` alone does
              neither: a flex item's automatic minimum is its min-content, and
              min-content for `white-space: nowrap` text is the whole string, so
              two truncating siblings simply pushed each other past the box and
              the status word was cut at the card's edge at 360 — `runni`. The
              title keeps a floor because it names the lane; the subtitle is the
              one that gives. */}
          <strong className="min-w-24 flex-1 truncate text-sm">{title}</strong>
          {subtitle && <span className="min-w-0 truncate font-mono text-2xs text-ink-faint">{subtitle}</span>}
        </div>
      ...
      <div
        ...
        aria-live="polite"
        aria-label={title}
        // Never `dvh`: on iOS the dynamic viewport unit re-measures as the
        // address bar collapses, so a pane sized in it resizes under a thumb
        // that is scrolling it. `--app-height` is the shell's own measured
        // height and does not move (`app/shell/viewport.ts`).
        className="live-body h-[min(380px,calc(var(--app-height,100%)*0.5))] overflow-y-auto px-3 py-2 font-mono text-2xs leading-relaxed"
        onScroll={(event) => {
          if (needle) return;
          ...
          shown.map((line) => (
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  // The SHARED copy — it reports what happened. The bare
                  // `navigator.clipboard?.writeText` it replaces failed
                  // silently wherever the API is refused, and the line lit up
                  // as though it had worked.
                  const href = new URL(lineHref(line.id), window.location.href).toString();
                  void copy(href, 'Link copied');
                  setLinked(line.id);
                }}
              >
