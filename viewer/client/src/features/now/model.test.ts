...
describe('which lanes are running now', () => {
  ...
  it('draws a done phase whose QA round is in flight, marked as the round it is', () => {
    const lanes = nowLanes([
      run({
        phases: {
          '1': record({ phase: 1, status: 'done' }) as never,
          '3': record({
            phase: 3,
            status: 'done',
            qaSession: { round: 1, report: 'reports/phase-03-qa.md', startedAt: '2026-09-07T04:00:00.000Z' },
          }) as never,
        },
      }),
    ]);
    expect(lanes.map((l) => l.phase)).toEqual([3]);
    expect(lanes[0]?.qa).toEqual({ round: 1 });
  });

  it("carries the session's own task list onto the lane", () => {
    // What the lane row and the Sessions row both render as `3/7 · …`. Carried
    // rather than fetched per row: every surface that draws a lane already
    // holds the record it comes from.
    const tasks = [
      { id: 'p4.task1', content: 'one', status: 'completed' },
      { id: 'p4.task2', content: 'two', activeForm: 'Doing two', status: 'in_progress' },
    ];
    const [withList] = nowLanes([run({ phases: { '4': record({ tasks }) as never } })]);
    expect(withList?.tasks).toEqual(tasks);
    // Absent, not empty, when the session published none — the surfaces read
  ...
  it('ranks a FROZEN lane as frozen, not as stalled — the silence was ordered', () => {
    // The inverse of the test above, and the reason it needed one: a frozen
    // lane is silent by construction, so the clock that correctly promotes a
    // wedged lane promoted every deliberate freeze into the attention band too
    // — with a stall episode still on its record from the seconds before the
    // freeze landed. Freezing something must not make the console call it broken.
    const now = Date.UTC(2026, 7, 20, 10, 0);
    const lanes = nowLanes(
      [
        run({
          freeze: {
            at: '2026-08-20T09:20:00.000Z',
            phase: 4,
            pid: 41,
            by: 'console',
            escalateAt: '2026-08-20T09:35:00.000Z',
          },
          phases: {
            '2': record({ phase: 2, startedAt: '2026-08-20T06:00:00.000Z' }) as never,
            '4': record({
              ...
              liveness: {
                phase: 4,
                lastOutputAt: '2026-08-20T09:20:00.000Z',
                turnsSinceLastTool: 0,
                commitsSinceStart: 0,
                treeDirty: false,
                stall: { signal: 'silent', since: '2026-08-20T09:20:00.000Z', detail: 'nothing' },
    ...
    const frozen = lanes.find((l) => l.phase === 4)!;
    expect(frozen.frozen).toBe(true);
    expect(laneSilent(frozen, now)).toBe(false);
  });
...
describe('filtering and load', () => {
  ...
  it('hides claimed phases, and gated phases, on request', () => {
    expect(
      applyDepartureFilters(queue(), { ...NO_DEPARTURE_FILTERS, unclaimed: true }).map((d) => d.slug),
    ).toEqual(['b']);
    expect(
      applyDepartureFilters(queue(), { ...NO_DEPARTURE_FILTERS, open: true }).map((d) => d.slug),
    ).toEqual(['a']);
  });
