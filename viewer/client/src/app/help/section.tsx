...
import { scrollBehavior, usePhone } from '@/lib/media';
 ...
 *
 * The glide is a preference, not a constant. `theme.css`'s blanket
 * reduced-motion rule flattens CSS `scroll-behavior`, but this call passes
 * `behavior` explicitly and CSSOM-View says an explicit value WINS over the
 * computed property — so the one scroll the app animates itself was the one
 * place the preference did not reach. `scrollBehavior()` asks the media query.
 */
function reveal(id: string): void {
  ...
  pane.scrollTo({ top, behavior: scrollBehavior() });
}
...
export function SectionPanel({
  ...
  card?: string;
  /**
   * The permalink for one card, built by the caller — which is the only place
   * that knows the route the overlay was opened OVER. See `GuideCard`.
   */
      ...
      {banded ? (
        outline.groups.map((group) => (
          <section key={group.id} className="flex flex-col gap-3">
            ...
            <CardList cards={group.cards} hrefFor={hrefFor} route={route} open={open} setOpen={setOpen} />
          </section>
        ))
      ) : (
        <CardList cards={cards} hrefFor={hrefFor} route={route} open={open} setOpen={setOpen} />
      )}
    </div>
  );
}
...
function CardList({
  ...
  /** One card's permalink — see `SectionPanel`. */
  hrefFor: (cardId: string) => string;
  route: boolean;
  open: Set<string>;
