...
import { type QA_RESULTS } from '../../../shared/plan-vocab.js';
...
/**
 * Why a phase stopped, in words — the reason axis PRINTED rather than merely
 * painted.
 *
 * The colour says how much it matters; this says what it is. A `parked` phase
 * is six different situations wearing one word, and "parked" alone is exactly
 * the sentence an operator has to open the run page to understand.
 */
const STOP_KIND_HELP: Record<string, string> = {
  gate: 'its gate is not clear and the console may not clear it',
  'human-check': 'verification passed and a person still owes the sign-off',
  'scope-cap':
...
/**
 * The two stop kinds whose sentence CONTRADICTS the status word's own blurb —
 * `parked` opens "Needs a person", and these are the parks where nobody is.
 */
const NOT_AN_ASK = new Set(['scope-cap', 'mcp']);

export function phaseStatusTitle(
  status: string | undefined,
  stop?: { kind?: string; declared?: string } | null,
): string | undefined {
  const why = stop?.kind ? STOP_KIND_HELP[stop.kind] : undefined;
  const base = line(PHASE_STATUS_HELP[status as PhaseStatus]);
  if (!why) return base;
  const detail = stop?.declared ? `${why} (${stop.declared})` : why;
  // The status blurb is REPLACED, not appended to, for the two parks that are
  // not an ask. `PHASE_STATUS_HELP.parked` opens "Needs a person…", so joining
  // them produced a tooltip that contradicted itself in one sentence —
  // "Needs a person… — nobody is being asked for anything" — which is worse
  // than either half alone. Where the reason disagrees with the generic word,
  // the reason is the truer sentence and stands on its own.
  if (NOT_AN_ASK.has(stop?.kind ?? '')) return detail;
...
/**
 * The four WRITABLE verdicts, which is what this help table explains.
 *
 * ⚠️ Not to be confused with `shared/plan-vocab.js`'s `QA_RESULT_WORDS`,
 * which is these four PLUS `unknown` — the word a `test-status.md` row that
 * will not parse reads as. There is no help text for `unknown` because it is
 * not a verdict anybody wrote; it is a parse failure.
 */
export type QaResultWord = (typeof QA_RESULTS)[number];
