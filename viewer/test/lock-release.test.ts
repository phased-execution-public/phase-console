...
function claim(
  root: string, slug: string, phase: number, owner: string, leaseFromNowS: number, session?: string,
): string {
...
test('a live lease whose session the registry has never seen is still refused', async () => {
  // `unknown` is NOT `ended`, and this is the clause that must never soften. A
  // stopped or zombie holder, a lock with no `session=` line, and a session
  // this console simply has not seen all answer `unknown` — and releasing one
  // of those is how a live session's lock got taken out from under it. Only an
  // ENDED session makes an unexpired claim debris.
  const { root, cleanup } = scratchDocs();
  try {
    addPlan(root, 'demo');
    const file = claim(root, 'demo', 3, 'someone/working', 1800, 'a-session-nobody-here-has-seen');
    const svc = service(root);
    try {
      const result = await svc.releaseLock('demo', 3);
      assert.equal(result.ok, false, 'an unknown session leaves the lease in charge');
      assert.match(result.detail!, /still working/);
      assert.equal(existsSync(file), true, 'and the claim survives');
    } finally { svc.close(); }
  } finally { cleanup(); }
});
