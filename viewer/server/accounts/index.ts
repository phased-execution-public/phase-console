...
import { liveBuckets, UsagePoller, type AccountUsage, type TokenAnswer } from './usage.ts';
import { AUTH_STATES } from '../../shared/ops-vocab.js';
...
export class Accounts {
  ...
  /**
   * ONE path for every kind. The default account used to branch away into a
   * volatile field here, which is precisely why its walls did not survive a
   * restart; the store now mints it a reserved row (`store.markLimited`).
   */
  ...
  limitedUntil(accountId: string | undefined, nowMs = Date.now()): Record<string, string> {
    const id = accountId ?? DEFAULT_ACCOUNT_ID;
    const raw = this.store.get(id)?.limitedUntil ?? {};
