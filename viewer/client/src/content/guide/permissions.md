...
auto-granted card is recorded in the queue's history, marked *decided by auto-grant*, and written to
the run's journal (`phase.tool-auto-granted`), so the audit trail is what it always was; only the
waiting is gone. The deny list is untouched — auto-grant answers only what classified `ask` — and a
wrapper hiding something the deny list would stop still gets a person.
