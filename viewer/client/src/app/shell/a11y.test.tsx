...
describe('the navigation names itself', () => {
  ...
  it('announces the rail bands, which it draws as a hairline and nothing else', () => {
    // The grouping is real information — the work, the record, the console —
    // and on the rail it is carried ENTIRELY by a 1px rule. Without a named
    // group it is the one reading a screen reader cannot get (WCAG 1.3.1).
    mount(<Rail state={STATE} counts={COUNTS} head="now" />);
    expect(screen.getAllByRole('group').map((el) => el.getAttribute('aria-label'))).toEqual([
      'The work',
      'The record',
      'The console',
    ]);
  ...
  it('names the More sheet bands from the labels it already draws', () => {
    // Here the words ARE on screen, so the group points at them rather than
    // repeating them — `aria-labelledby`, not a second `aria-label`.
    mount(
      <MoreSheet open onOpenChange={() => {}} state={STATE} counts={COUNTS} route={route()} head="now" />,
    );
    // Queried BY NAME, which is the whole claim. Matching on `textContent`
    // instead — as the first version of this test did — stays green when
    // `aria-labelledby` is deleted and only the role survives, because the
    // label is inside the group either way. Measured: that weaker form passed
    // the mutation it existed to catch. The tab bar takes the whole work band,
    // so the sheet holds the other two.
    expect(screen.getByRole('group', { name: 'The record' })).toBeTruthy();
    expect(screen.getByRole('group', { name: 'The console' })).toBeTruthy();
    expect(screen.queryByRole('group', { name: 'The work' })).toBeNull();
  });
