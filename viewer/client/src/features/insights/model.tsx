  ...
  return (
    <div className="grid min-w-0 gap-3 md:grid-cols-2 lg:grid-cols-3">
      <Card>
        ...
        <CardBody>
          {stats.repos.length ? (
            <BarList label="repos" items={stats.repos.map((row) => ({ name: row.repo, value: row.count }))} />
          ) : (
      ...
      <Card>
        ...
        <CardBody>
          {stats.skills.length ? (
            <BarList
              label="skills"
              items={stats.skills.map((row) => ({ name: row.skill, value: row.count }))}
            />
          ) : (
      ...
      <Card>
        ...
        <CardBody>
          {stats.models.length ? (
            <BarList
              label="models"
              items={stats.models.map((row) => ({ name: row.model, value: row.count }))}
            />
          ) : (
            <p className="text-sm text-ink-faint">
