...
describe('filtering', () => {
  ...
  /*
   * The badge on the phone's Sort & filter button. It is derived from
   * `NO_FILTERS` by the filter kit rather than re-listed, because the copy it
   * replaces had to be edited every time a control was added — and the way
   * that fails is a badge that silently stops counting one.
   *
   * The search field is deliberately not counted: it is on screen carrying its
   * own text, and counting it again would double-report what the operator is
   * already looking at.
   */
  it('counts the filters that are away from their resting value, and not the search box', () => {
    expect(activeFilterCount(NO_FILTERS)).toBe(0);
    expect(activeFilterCount({ ...NO_FILTERS, query: 'cart' })).toBe(0);
    expect(activeFilterCount({ ...NO_FILTERS, showClosed: true })).toBe(1);
    expect(activeFilterCount({ ...NO_FILTERS, showDocuments: true, repo: 'hub' })).toBe(2);
    expect(
      activeFilterCount({
        ...NO_FILTERS,
        showDocuments: true,
...
describe('the plans page', () => {
  ...
  //
  // It was answered with a banner, and is answered with a FOLD now: the estate
  // is on the page rather than announced as missing from it, its count is drawn
  // while folded, and one press puts every row on screen. That is strictly more
  // than the banner did — a banner can be dismissed, and a section cannot be.
  it('keeps the closed estate on the page, folded, with its count on the fold', async () => {
    plans.mockResolvedValue([
    ...
    // Folded: named, counted, and the rows are NOT in the document yet.
    const fold = screen.getByRole('button', { name: /^Closed plans/ });
    expect(fold).toHaveAttribute('aria-expanded', 'false');
    expect(within(fold).getByText('(8)')).toBeTruthy();
    expect(screen.queryByText('Finished 0')).toBeNull();
  ...
  it('stops calling the folded plans hidden — the page would be arguing with itself', async () => {
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Still going' }),
      ...Array.from({ length: 3 }, (_, i) =>
        plan({ slug: `done-${i}`, title: `Finished ${i}`, status: 'complete' }),
      ),
      plan({ slug: 'notes', title: 'Notes', kind: 'document', phases: 0, ready: [] }),
    ]);
    const { default: PlansView } = await import('./index');
    ...
    // Each clause says what actually happened to ITS rows: closed plans are on
    // the page, three inches below this line; a document has no phases, so no
    // row shape to fold into, and is genuinely left out.
    expect(screen.getByText(/3 closed, folded below/)).toBeTruthy();
    expect(screen.getByText(/1 document hidden/)).toBeTruthy();
    // …and the fold it points at is really there, with the same count.
    const fold = screen.getByRole('button', { name: /^Closed plans/ });
    expect(within(fold).getByText('(3)')).toBeTruthy();
  });

  it('says the matches are closed rather than that everything is filtered out', async () => {
    // The page used to read "Every plan is filtered out" three lines above a
    // live "Closed plans (1)" fold — two surfaces disagreeing about whether
    // anything matched, which is the same defect as the note that went on
    // calling a fold hidden.
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Still going' }),
    ...
    fireEvent.change(screen.getByRole('searchbox'), { target: { value: 'walked' } });
    expect(await screen.findByText('Every match is a closed plan')).toBeTruthy();
    expect(screen.queryByText('Every plan is filtered out')).toBeNull();
    // …and the fold it points at is really there, with the match in it.
    fireEvent.click(screen.getByRole('button', { name: /^Closed plans/ }));
    expect(await screen.findByText('Walked away')).toBeTruthy();
  });

  it('reaches the plans on a source where EVERY plan is closed', async () => {
    // The branch this is about renders no list at all — it used to be a
    // sentence about the whole source and no way to see one row of it. The
    // fold is rendered there too, so the estate is reachable from the one page
    // state where it is the entire estate.
    plans.mockResolvedValue([
      plan({ slug: 'a', title: 'Finished one', status: 'complete' }),
      plan({ slug: 'b', title: 'Walked away', status: 'abandoned' }),
    ]);
    const { default: PlansView } = await import('./index');
    mount(<PlansView />);
    expect(await screen.findByText('Every plan here is closed')).toBeTruthy();

    const fold = screen.getByRole('button', { name: /^Closed plans/ });
    expect(within(fold).getByText('(2)')).toBeTruthy();
    fireEvent.click(fold);
    expect(await screen.findByText('Finished one')).toBeTruthy();
    expect(screen.getByText('Walked away')).toBeTruthy();
  });

  it('narrows the closed estate with the same search that narrows the list', async () => {
    // The fold is measured through the real filter with one flag moved, so a
    // query typed into the toolbar has to reach it. A section that ignored the
    // search would be a second, contradictory answer to "what is in here".
    // The open plan matches the query too, deliberately: an empty main list
    // takes the "everything is filtered out" branch, and this is about the fold
    // narrowing beside a list, not about that branch.
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Pear orchard' }),
      plan({ slug: 'done-apple', title: 'Apple pie', status: 'complete' }),
      plan({ slug: 'done-pear', title: 'Pear jam', status: 'complete' }),
    ]);
    const { default: PlansView } = await import('./index');
    mount(<PlansView />);
    await screen.findByText('Pear orchard');

    fireEvent.click(screen.getByRole('button', { name: /^Closed plans/ }));
    expect(await screen.findByText('Apple pie')).toBeTruthy();
    expect(screen.getByText('Pear jam')).toBeTruthy();

    fireEvent.change(screen.getByRole('searchbox'), { target: { value: 'pear' } });
    await waitFor(() => expect(screen.queryByText('Apple pie')).toBeNull());
    expect(screen.getByText('Pear jam')).toBeTruthy();
  });

  it('folds nothing when the closed rows are already in the list', async () => {
    // `Show closed` puts them above; a fold repeating them would be the same
    // plans counted twice on one page.
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Still going' }),
      plan({ slug: 'gone', title: 'Walked away', status: 'abandoned' }),
    ]);
    setPrefs({ showClosed: true });
    const { default: PlansView } = await import('./index');
    mount(<PlansView />);
    await screen.findByText('Walked away');
    expect(screen.queryByRole('button', { name: /^Closed plans/ })).toBeNull();
  });

  it('folds nothing when a STATUS filter has already put the closed rows above', async () => {
    // The toggle is not the only thing that puts them there. `showClosed`'s
    // predicate stands down whenever the status control names a closure — so
    // with `closed only` selected the toggle is inert, the list holds every
    // closed plan, and a fold guarded on the toggle repeated all of them. On a
    // 72-plan source that drew 142 rows for 71 plans.
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Still going' }),
      plan({ slug: 'a', title: 'Finished one', status: 'complete' }),
      plan({ slug: 'b', title: 'Walked away', status: 'abandoned' }),
    ...
    expect(await screen.findByText('Walked away')).toBeTruthy();
    // Each closed plan exactly once, and no fold under them.
    expect(screen.getAllByText('Walked away')).toHaveLength(1);
    expect(screen.getAllByText('Finished one')).toHaveLength(1);
    expect(screen.queryByRole('button', { name: /^Closed plans/ })).toBeNull();
  });

  it('says a DOCUMENT is not listed — the one shape that has no fold', async () => {
    // The band speaks only for documents now. A document has no phases, so it
    // has no row shape to fold into, and it is genuinely absent rather than
    // folded — which is the whole reason it still gets a sentence.
    plans.mockResolvedValue([
      plan({ slug: 'live', title: 'Still going' }),
      ...Array.from({ length: 8 }, (_, i) =>
        plan({ slug: `doc-${i}`, title: `Note ${i}`, kind: 'document', phases: 0, ready: [] }),
      ),
    ]);
    const { default: PlansView } = await import('./index');
    mount(<PlansView />);
    await screen.findByText('Still going');
    expect(screen.getByText(/8 documents are not listed/)).toBeTruthy();
    // No closed rows in this source, so nothing to fold.
    expect(screen.queryByRole('button', { name: /^Closed plans/ })).toBeNull();

    // 28 px is a comfortable target for a pointer and a miss for a thumb, and
    // this is the one control on the band that is not a word.
    const dismiss = screen.getByRole('button', { name: /^Dismiss/ });
    expect(dismiss.className).toContain('[@media(hover:none)]:size-(--tap-min)');
    ...
    // must not be the same as being told nothing.
