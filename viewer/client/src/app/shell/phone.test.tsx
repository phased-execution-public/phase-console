...
describe('the announcement count in the header', () => {
  it('sits beside the bell rather than on top of it', () => {
    render(<Header state={undefined} counts={counts({ unread: 4 })} route={route()} phone />);
  ...
  it('keeps three digits on the screen', () => {
    render(<Header state={undefined} counts={counts({ unread: 1284 })} route={route()} phone />);
  ...
  it('draws nothing at all when there is nothing unread', () => {
    render(<Header state={undefined} counts={counts()} route={route()} phone />);
...
describe('the asks chip', () => {
  it('opens the inbox panel and disappears at zero', () => {
    const first = render(<Header state={undefined} counts={counts({ asks: 2 })} route={route()} phone />);
    const chip = screen.getByRole('button', { name: 'Waiting on you, 2 asks' });
