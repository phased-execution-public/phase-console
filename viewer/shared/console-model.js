...
import { MAX_TASKS, adoptTaskId, foldTaskEvent, tasksFromList } from './task-model.js';
...
/**
 * Rows in the task-list panel.
 *
 * Re-exported, never re-declared: this was 80 here and 60 in `spawn.ts`, so a
 * long list was silently two different lengths depending on which end of the
 * wire you asked. One number, in `shared/task-model.js`, imported by both.
 */
export const MAX_TODOS = MAX_TASKS;
...
/**
 * `430 ms` / `4.2 s` / `11 m` — a duration for one log line, with no dependency.
 *
 * The client has `lib/format.ts` and the server has none, and this file is
 * imported unmodified by both; four lines of arithmetic are cheaper than a
 * shared module that exists to hold them.
 */
function msWords(ms) {
  const n = Number(ms);
...
export function toLine(event, data) {
  if (event === 'verify') {
    // Four shapes on one channel, discriminated by which field is present:
    // a start (`command` alone), a settled command (`ok`), the skips this
    // machine forced (`skipped`), and the verdict (`summary`). The start shape
    // is byte-for-byte what it always was, so a transcript recorded before the
    // other three replays exactly as it used to.
    if (Array.isArray(data.skipped)) {
      const leads = [...new Set(data.skipped.map((s) => s.lead))].join(', ');
      return {
        kind: 'verify',
        text:
          `${data.skipped.length} command${data.skipped.length === 1 ? '' : 's'} skipped — ` +
          `not installed on this machine (${leads})`,
      };
    }
    if (data.summary) {
      const s = data.summary;
      return { kind: 'verify', text: `verification ${s.ok ? 'passed' : 'failed'} — ${s.reason}` };
    }
    if (typeof data.ok === 'boolean') {
      return {
        kind: 'verify',
        text:
          `[${data.index + 1}/${data.total}] ${data.ok ? 'passed' : `failed, exit ${data.code}`}` +
          ` in ${msWords(data.ms)}${data.retry ? ' (on retry)' : ''}`,
      };
    }
  ...
  switch (data.kind) {
    ...
    // The transcript talking about ITSELF — that it is shedding streamed
    // fragments, that it is full. A reader who is not told simply sees the
    // output thin out and then stop, and concludes the session did.
    case 'notice':
      return { kind: 'runner', text: String(data.text ?? '').trim() };
    default:
      return null;
  }
}
...
export const NO_ACTIVITY = Object.freeze({
  ...
  todos: [],
  /** When the session last REWROTE that list — the panel's "written at". */
  todosAt: 0,
  ...
  agents: [],
  /**
   * The §Verification, as a checklist rather than as a verdict that arrives at
   * the end. `commands` is indexed by the runner's own `index`, so a start and
   * its result land on one row.
   */
  verify: Object.freeze({ commands: [], skipped: [], summary: null, at: 0 }),
});

/** The empty checklist — also what a state built by an older build lacks. */
const NO_VERIFY = NO_ACTIVITY.verify;

/**
 * What a row of the verification checklist can BE — worst-first, and written
 * here because `foldVerify` below is the only thing that assigns one.
 *
 * `unknown` is not a runner outcome. It is what a row that never reported one
 * becomes when the verdict lands — a stop mid-verification, most often — and it
 * exists so the checklist can decline to guess rather than leaving a row
 * claiming it is still running under a verification that is over.
 */
export const VERIFY_STATES = Object.freeze(['failed', 'running', 'skipped', 'unknown', 'passed']);
...
export function activity(state, event, data, at = Date.now()) {
  ...
  // The verification runs AFTER the session ends, so it arrives on its own
  // channel rather than in the transcript — and it is state, not a log: the
  // interesting reading is "which command is in flight and what did the last
  // one do", which a scrolling console can only answer by scrolling.
  if (event === 'verify' && data) return foldVerify(state, data, at);

  // The phase LEFT FLIGHT, so its verification is over whatever the checklist
  // still says. Without this, every transcript recorded before the settled and
  // summary events existed replays with its rows stuck on `running`: the old
  // stream carried starts and nothing else, so `foldVerify` opened a row per
  // command and nothing ever closed one — and opening a finished run's lane
  // pane drew a card claiming both commands were still going on a run that
  // ended days ago. `running` and `verifying` are excluded because they are the
  // phase IN flight; the reset above has already handled a new phase starting.
  if (event === 'phase' && data?.status && data.status !== 'running' && data.status !== 'verifying') {
    return settleVerify(state);
  }

  if (event !== 'stream' || !data) return state;

  switch (data.kind) {
    // THREE spellings of one thing, folded by one function.
    //
    // `TodoWrite` rewrites the whole list; `TaskCreate`/`TaskUpdate` build it a
    // row at a time; and `scripts/phase-tasks.sh` — the channel the CLI cannot
    // take away — writes the same transitions to a file the runner tails and
    // replays here as `task` events. The rules live in `shared/task-model.js`
    // so the SERVER's fold (which sees every line, and is what seeds this panel
    // on load) and this one cannot build different lists.
    case 'todos': {
      // An empty array is a legitimate, meaningful write: a whole-list rewrite
      // saying the session finished or abandoned its plan. It used to be read
      // as "nothing to see", so a cleared list left the last one on the screen
      // for ever (r2 client-15). Only a NON-array means "not a task write".
      const items = tasksFromList(data.items);
      if (!items) return state;
      if (!items.length && !state.todos.length) return state;
      return { ...state, todos: items, todosAt: at };
    }

    case 'task': {
      const todos = foldTaskEvent(state.todos, data);
      // The identity contract: `foldTaskEvent` returns the array it was given
      // when nothing changed, and passing the same `state` back is what makes
      // React bail out instead of re-rendering three panels.
      return todos === state.todos ? state : { ...state, todos, todosAt: at };
    }
    ...
    case 'tool-result': {
      ...
      // A `TaskCreate`'s id is in its RESULT, not its input, so this is the
      // only moment a created row can learn the id every later update will
      // name it by. The rule itself is in `shared/task-model.js` because the
      // server's fold needs the same one.
      const todos = adoptTaskId(state.todos, data.id, data.detail);
      ...
      if (index < 0 && !closes && todos === state.todos) return state;
...
/**
 * Fold one `verify` event into the checklist.
 *
 * The four shapes `toLine` discriminates, read as state instead of as lines:
 * a start opens a row `running`, its result closes the same row (matched on the
 * runner's own `index`, never on the command text — two identical commands in
 * one bullet are two rows), the skips arrive once before anything runs, and the
 * summary closes the list.
 *
 * Obeys the identity contract: an event that changes nothing returns `state`.
 */
/**
 * Close any row still reading `running` — the phase is over, however it ended.
 *
 * `unknown` rather than a guess: the row must not claim an outcome the runner
 * never gave it. Returns `state` when there is nothing open, per the identity
 * contract.
 */
function settleVerify(state) {
  const was = state.verify ?? NO_VERIFY;
  if (!was.commands.some((entry) => entry.state === 'running')) return state;
  return {
    ...state,
    verify: {
      ...was,
      commands: was.commands.map((entry) =>
        entry.state === 'running' ? { ...entry, state: 'unknown' } : entry,
      ),
    },
  };
}

/** Deep-equal for one checklist row — what the identity contract is asked about. */
...
function foldVerify(state, data, at) {
  const was = state.verify ?? NO_VERIFY;
  ...
  if (Array.isArray(data.skipped)) {
    ...
    if (!skipped.length) return state;
    // A re-sent skip list is the transcript replaying, not news. The identity
    // contract is the whole reason this file's header exists: `at` alone
    // changing must not allocate a new object and re-render three panels.
    if (skipped.length === was.skipped.length && skipped.every((entry, i) => sameRow(entry, was.skipped[i])))
      return state;
    return { ...state, verify: { ...was, skipped, at } };
  }

  if (data.summary && typeof data.summary === 'object') {
    const summary = {
      ok: data.summary.ok === true,
      reason: typeof data.summary.reason === 'string' ? data.summary.reason : '',
      ran: Number(data.summary.ran) || 0,
      notRun: Number(data.summary.notRun) || 0,
      skipped: Number(data.summary.skipped) || 0,
    };
    if (sameRow(summary, was.summary)) return state;
    // Anything still reading `running` when the verdict lands never reported
    // one — a stop mid-verification, most often. `unknown` rather than a guess:
    // the row must not claim an outcome the runner did not give it.
    const commands = was.commands.map((entry) =>
      entry.state === 'running' ? { ...entry, state: 'unknown' } : entry,
    );
    return { ...state, verify: { ...was, commands, summary, at } };
  }

  const index = Number(data.index);
  if (!Number.isInteger(index) || index < 0) return state;
  const total = Number(data.total) || 0;
  const settled = typeof data.ok === 'boolean';
  const row = {
    index,
    total,
    command: typeof data.command === 'string' ? data.command : '',
    state: settled ? (data.ok ? 'passed' : 'failed') : 'running',
    at,
    ...(settled && Number.isFinite(Number(data.ms)) ? { ms: Number(data.ms) } : {}),
    ...(settled && Number.isFinite(Number(data.code)) ? { code: Number(data.code) } : {}),
    ...(data.retry ? { retry: true } : {}),
    ...(typeof data.tail === 'string' && data.tail ? { tail: data.tail } : {}),
  };
  const found = was.commands.findIndex((entry) => entry.index === index);
  // A duplicate start or a re-sent result — the replay's shape — changes
  // nothing, so it must return the state it was given. Asked WHATEVER the
  // summary says: guarding this on `summary === null` left a re-sent identical
  // result falling through to the write below, which cleared a verdict that had
  // already landed.
  if (found >= 0 && sameRow({ ...was.commands[found], at: row.at }, row)) return state;
  const commands =
    found < 0
      ? [...was.commands, row].sort((a, b) => a.index - b.index)
      : // The start carried the command text; the result carries it too, but a
        // future producer that omitted it must not blank a row that had it.
        was.commands.map((entry, i) =>
          i === found ? { ...entry, ...row, command: row.command || entry.command } : entry,
        );
  // A row genuinely CHANGING after the verdict means the verdict is stale — a
  // re-verification, most often — so it is dropped. An unchanged row never
  // reaches here.
  return { ...state, verify: { ...was, commands, summary: null, at } };
}
