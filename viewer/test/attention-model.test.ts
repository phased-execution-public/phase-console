...
test('the stall SIGNALS are a different list from the stall KINDS, on purpose', () => {
  ...
  // `external-wait` outranks `silent` for the same reason `retrying` does: a
  // lane inside `gh run watch` IS silent, and this names the cause — which is
  // the difference between "go and look at it" and "park it, it is waiting".
  assert.ok(
    STALL_SIGNALS.indexOf('external-wait') < STALL_SIGNALS.indexOf('silent'),
    'a waiting lane must not be reported as merely silent',
  );
  assert.ok(
    STALL_SIGNALS.indexOf('retrying') < STALL_SIGNALS.indexOf('external-wait'),
    'a session that cannot reach the API at all is the harder fact',
  );
  // The bridge is exactly two pairs: a lane whose signal is `silent` raises the
  // `session-silent` row, and one whose signal is `retrying` raises
  // `session-retrying` — each at that row's own, longer clock.
  ...
  assert.ok(STALL_KINDS.includes('session-retrying'));
  // `retrying`'s knob is a COUNT, so the floor is compared against the
  // shortest storm that can reach the detector at the CLI's ~30s cadence.
  assert.ok(STALL_META['session-retrying'].afterMs > STALL_DEFAULTS.stallRetryBurst * 30_000,
    'the inbox floor must outlast the burst that raises the signal');
});
...
test('the watchdog clocks sit BESIDE the detector map, and fire in the one right order', async () => {
  const {
    STALL_NUDGE_GRACE_MS, SPAWN_FIRST_EVENT_MS,
  } = await import('../shared/attention-model.js');

  // The bijection above is the reason this test exists. `STALL_DEFAULTS` is
  // exactly the detector thresholds — one per signal, pinned two ways — so a
  // remedy clock added to it would break `stallThresholds()` AND the
  // signal/pref pairing, and the tempting repair is to weaken one of them.
  // Same ruling `STALL_ESCALATE_MS` already carries.
  assert.ok(!('stallNudgeGraceMs' in STALL_DEFAULTS));
  assert.ok(!('spawnFirstEventMs' in STALL_DEFAULTS));

  // The order is the whole safety argument of the ladder. The runner nudges at
  // the silent threshold, recycles a grace later, and only a lane with no
  // runner behind it — no lane, no ticker, no console driving — ever reaches
  // the spawn backstop. A backstop that fired FIRST would take the remedy away
  // from the layer that can keep the session.
  assert.ok(STALL_NUDGE_GRACE_MS > 0);
  assert.equal(
    SPAWN_FIRST_EVENT_MS, STALL_DEFAULTS.stallSilentMs + 2 * STALL_NUDGE_GRACE_MS,
    'the backstop is derived from the other two, so it can never drift under them',
  );
  assert.ok(
    SPAWN_FIRST_EVENT_MS > STALL_DEFAULTS.stallSilentMs,
    'the runner must get to nudge before anything kills the child',
  );
  assert.ok(
    // STRICTLY later, not level with it. At exactly the recycle threshold the
    // two clocks race on the same tick, and whichever wins decides whether the
    // session is kept or thrown away — a coin toss over the one property the
    // recycle exists to preserve.
