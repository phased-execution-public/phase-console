...
export function labelledBullets(block: string): Bullet[] {
  ...
  for (const line of lines) {
    ...
    // An UNLABELLED bullet continues the open field, at any indent.
    //
    // It used to close the field at indent 0 and throw the line away — neither
    // kept in the parent's body nor emitted as a bullet of its own — so the
    // commonest way to write a list of commands,
    //
    //     - **Verification:**
    //     - `npm test`
    //     - `npm run lint`
    //
    // gave Verification an EMPTY body. The engine reads the same plan happily:
    // `_verification_reach` sets its flag on the Verification bullet and then
    // takes every backtick-carrying line to the end of the phase block, so F14
    // stayed quiet and validate.sh exited clean while the console's preflight
    // found nothing runnable and parked the phase at boarding — the two sides
    // disagreeing about a plan neither of them thought was malformed.
    //
    // A field now runs until the next LABELLED top-level bullet (or the end of
    // the block), which is the boundary the author actually wrote. That stays
    // narrower than the engine's reach-to-end-of-block, and deliberately so:
    // this reading is what the runner EXECUTES, so it must not swallow a later
    // field's commands the way F14's advisory scan safely can.
