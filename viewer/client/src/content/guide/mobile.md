...
you signed in with. `--allow-agent` is optional; it is what makes interactive sessions on the **Sessions**
page and the *New plan with AI* wizard work from the phone.
...
be able to set a session running. The payload carries no address either — only a signed token naming
...
Once the console is on your phone, the **Sessions** page — which is where both an interactive `claude`
and a plain shell live — is built for a thumb:
