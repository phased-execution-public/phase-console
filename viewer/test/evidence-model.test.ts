...
import {
  BOARD_WORDS, HANDOFF_WORDS, LIVE_VIA, QA_WORDS, RULING_KINDS, VERIFICATION_WORDS,
  deriveEvidence, isLiveVia, isQaWord, isVerificationWord,
...
test('nothing is named `done`: the claim and the evidence keep separate words', () => {
  ...
  // `stale` joins them for the same reason: the board's `in-progress` is also
  // a claim, and this is whether a process was found behind it.
  assert.deepEqual(
    Object.keys(e).sort(),
    ['board', 'evidenced', 'handoff', 'qa', 'stale', 'verification', 'why'],
  );
});

test('the vocabularies are frozen, complete and in decision order', () => {
  for (const v of [VERIFICATION_WORDS, QA_WORDS, BOARD_WORDS, HANDOFF_WORDS, RULING_KINDS, LIVE_VIA]) {
  ...
  // Falling authority: a probed process, then a lock the registry vouches for,
  // then the registry alone. `phaseLive` walks this order.
  assert.deepEqual([...LIVE_VIA], ['run', 'lock', 'registry']);
});
...
test('the client imports the SAME evidence model', async () => {
  ...
  assert.equal(client.LIVE_VIA, LIVE_VIA);
  assert.equal(client.isLiveVia, isLiveVia);
...
/* ------------------------------------------------------------------ *
 * `stale` — the OTHER claim, and the three-valued input behind it
 *
 * The board's `in-progress` says work is happening. It comes from a
 * `status: in-progress` line in a markdown file and nothing in the console
 * ever settled it against a process, which is how a phase of
 * one plan on this estate painted "Running" for 18 DAYS over no run, no
 * lock and no session. `live` is the measurement; `stale` is the verdict.
 * ------------------------------------------------------------------ */

const CLAIMS_RUNNING = { board: 'in-progress', handoff: { exists: true, status: 'in-progress' } };

test('an in-progress claim with nothing behind it is STALE, and says so', () => {
  const answer = deriveEvidence({ ...CLAIMS_RUNNING, live: null });
  assert.equal(answer.stale, true);
  assert.match(answer.why.join('\n'), /live: nothing is running this/);
  assert.match(answer.why.join('\n'), /the board word is a claim/);
});

test('an in-progress claim with a live fact is NOT stale, and names the witness', () => {
  for (const via of LIVE_VIA) {
    const answer = deriveEvidence({ ...CLAIMS_RUNNING, live: { via, session: 'sess-1' } });
    assert.equal(answer.stale, false, `${via} is a live fact`);
    assert.match(answer.why.join('\n'), /^live: /m);
    assert.match(answer.why.join('\n'), /sess-1/);
  }
  // Only `via: 'run'` ever knows a pid, and when it does it says so.
  assert.match(
    deriveEvidence({ ...CLAIMS_RUNNING, live: { via: 'run', pid: 4242 } }).why.join('\n'),
    /pid 4242/,
  );
});

test('AMBIGUITY 9: "looked and found nothing" and "did not look" are different inputs', () => {
  // The distinction the field exists for. A caller with no liveness dependency
  // (the drive loop) must not produce a warn chip nobody measured.
  const notLooked = deriveEvidence(CLAIMS_RUNNING);
  assert.equal(notLooked.stale, false);
  assert.match(notLooked.why.join('\n'), /live: not checked here/);

  const looked = deriveEvidence({ ...CLAIMS_RUNNING, live: null });
  assert.equal(looked.stale, true);

  // And the trap: `...(fact ? { live: fact } : {})` collapses to an ABSENT key
  // — and an explicit `undefined` must read the same way, because that is what
  // a conditional spread leaves behind. A caller that means "nothing" says
  // `null`.
  assert.equal(deriveEvidence({ ...CLAIMS_RUNNING, live: undefined }).stale, false);
});

test('only `in-progress` can be stale — `stuck`, `done`, `ready` and an unreadable board cannot', () => {
  // `stuck` claims a PERSON is needed, not that work is in flight. Badging it
  // "claimed running" would put a second warning on a state that already is
  // one. `done` and `ready` claim nothing about a process at all.
  for (const board of ['stuck', 'done', 'ready', 'waiting']) {
    assert.equal(deriveEvidence({ board, live: null }).stale, false, board);
  }
  // A board that could not be read makes no claim to be stale about.
  assert.equal(deriveEvidence({ boardError: 'the engine exploded', live: null }).stale, false);
});

test('an unreadable witness is a measurement that found nothing, and is quoted back', () => {
  const answer = deriveEvidence({ ...CLAIMS_RUNNING, live: { via: 'vibes' } as never });
  assert.equal(answer.stale, true, 'a witness we cannot name vouches for nothing');
  assert.match(answer.why.join('\n'), /"vibes" is not one of run\/lock\/registry/);
  assert.ok(isLiveVia('run') && !isLiveVia('vibes') && !isLiveVia(null));
});

test('`stale` is never folded into `evidenced`, and `why[]` is still never empty', () => {
  // Different questions about different phases: `evidenced` is about a `done`
  // claim and its proof, `stale` about an `in-progress` claim and its process.
  // A phase is one board word at a time, so they never both fire.
  const stale = deriveEvidence({ ...CLAIMS_RUNNING, live: null });
  assert.equal(stale.evidenced, false);
  assert.equal(stale.stale, true);

  const evidenced = deriveEvidence({ ...DONE, verification: V_GREEN, qa: { mode: 'off' }, live: null });
  assert.equal(evidenced.evidenced, true);
  assert.equal(evidenced.stale, false);

  for (const input of [stale, evidenced]) assert.ok(input.why.length > 0);
});
