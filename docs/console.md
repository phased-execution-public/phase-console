...
| Destination | Answers |
|---|---|
| **Now** | *Does anything need me, and what is running?* The needs-you inbox with inline actions (approvals, gates, errands, expired accounts, stalled lanes; a relayed **question** with one button per option and its countdown; the console's own reach as work — a notification nobody was told about, Tailscale down or serving another console; and an `fyi` **Policy answered** row for each call the policy table made in your name) — an action that takes your words (an answer, a new title) gets one box on the row, the same box `#/approve` draws — the live lanes with heartbeat, cost and ETA, each one's branch (its title naming the base it was cut from, the sha and who declared it), its tree and — on the runner's own word — that the tree is **locked**, what is next up across every plan, and the plans in flight. |
