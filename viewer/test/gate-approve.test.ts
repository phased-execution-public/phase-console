...
/* ------------------------------------------------------------------ *
 * P9 — the qa-record door's own refusals (issue #11)
 * ------------------------------------------------------------------ */

const qaPlan = (request: Record<string, unknown>) =>
  planWrite({ action: 'qa-record', slug: 'demo', phase: 2, ...request } as never, { root: '/tmp' });

test('P9: a waiver reason reaches the script at 280 characters, not 200', () => {
  // The client textarea, the `qa-waive` route and `qa-record.sh` all agree on
  // 280; this door silently cut at `REASON_MAX` (200) first, so an operator's
  // last eighty characters vanished between the box they typed in and the file
  // (QA round 1, L5). `REASON_MAX` still bounds a plan's `closed_reason` and a
  // gate note — different fields, different files.
  const reason = 'x'.repeat(300);
  const args = qaPlan({ result: 'waived', report: 'reports/phase-02-qa.md', reason }).args;
  const why = args[args.indexOf('--reason') + 1];
  assert.equal(why.length, 280);
});

test('P9: a reason is refused on anything but a waiver, and a round on a pending row', () => {
  // Refused with a SENTENCE here rather than by an exit code the browser would
  // render as "the write failed" — the same rule the script keeps.
  assert.throws(
    () => qaPlan({ result: 'pass', report: 'reports/phase-02-qa.md', reason: 'why' }),
    /only recorded with a waiver/,
  );
  assert.throws(
    () => qaPlan({ result: 'pending', report: 'reports/phase-02-qa.md', round: 2 }),
    /roundless/,
  );
  assert.throws(
    () => qaPlan({ result: 'pass', report: 'reports/phase-02-qa.md', round: 1_000_000 }),
    /six digits/,
  );
});

test('P9: a round the caller chose rides through verbatim — this door never numbers one', () => {
  // The only caller that passes a round is the QA-recovery loop, which has
  // already asked the ONE chooser. A second opinion about which round this is
  // has been the defect four times (Phase 4, QA rounds 1-4).
  const args = qaPlan({ result: 'fail', report: 'reports/phase-02-qa-round3.md', round: 3 }).args;
  assert.deepEqual(args.slice(-2), ['--round', '3']);
  // …and absent means absent: the script's own default decides.
  assert.ok(!qaPlan({ result: 'fail', report: 'reports/phase-02-qa.md' }).args.includes('--round'));
});
