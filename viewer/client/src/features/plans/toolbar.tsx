...
// `fieldSurface` straight from the primitive set rather than through
// `components/toolbar`'s `fieldClass` alias: these selects sit on the page
// ground, and there is one class for that.
import { Button, ButtonGroup, fieldSurface } from '@/components/ui';
import { Toolbar, ToolbarSorts, countedLabel, type ToolbarShape } from '@/components/toolbar';
 ...
 *
 * Measured against `NO_FILTERS` by the filter kit rather than re-listed here —
 * the copy that stood here had to be edited every time a control was added,
 * and the way that fails is a badge that stops counting a filter it does not
 * know about. The search field is excluded there (`counts: false`), because it
 * is on screen carrying its own text.
 */
export const activeFilterCount = activeCount;
      ...
      <div className={cn('flex gap-2', stacked && 'flex-col')}>
        {repos.length > 1 && (
          <label className="flex min-w-0 items-center gap-2 text-2xs text-ink-faint">
            ...
            <select
              className={cn(fieldSurface, stacked ? 'flex-1' : 'max-w-40')}
              value={filters.repo}
...
function HiddenNote({
  ...
  /*
   * Each clause says what actually happened to its rows, because they are no
   * longer the same thing. Closed plans are FOLDED — a section at the foot of
   * the page with its count on the fold — and a line still calling them hidden
   * would be the console disagreeing with a section three inches below it. A
   * document really is left out: it has no phases, so it has no row shape to
   * fold into.
   */
  const parts: string[] = [];
  if (hiddenBy.closed) parts.push(`${hiddenBy.closed} closed, folded below`);
  if (hiddenBy.documents)
    parts.push(`${hiddenBy.documents} ${hiddenBy.documents === 1 ? 'document' : 'documents'} hidden`);
  if (hiddenBy.search) parts.push(`${hiddenBy.search} hidden by the search`);

  return (
    <>
      {parts.join(' · ') || `${hiddenBy.total} hidden`}
      {(hiddenBy.closed > 0 || hiddenBy.documents > 0) && (
        <>
          ...
          <button
            ...
            className="tap-line underline underline-offset-2 hover:text-action"
          >
            show everything
          </button>
        </>
