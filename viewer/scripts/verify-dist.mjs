...
let ok =
  ...
  // Before check-dist, and not optional: the first-paint gate measures the
  // bytes the SERVER would send, so a scratch build with no `.br` siblings
  // would be judged on its identity size and fail a budget the real build
  // passes. `npm run build` runs these two in the same order.
  run('precompress', process.execPath, [join(VIEWER, 'scripts', 'precompress.mjs')]) &&
