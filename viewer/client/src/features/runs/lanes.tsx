 ...
 * 6 doing".
 *
 * Tab state is in the component; the OPENING tab may come from the route.
 * `#/plan/:slug/run` addresses the RUN, and a lane is a thing that exists for
 * twenty minutes — so `?lane=p7` is a hint, not an address: it decides which
 * tab opens, `resolveTab` drops it the moment that lane is gone, and clicking
 * another tab does not rewrite the URL. Without it the pane an operator was
 * pointed at from a phase chip could not be linked to at all, which is B2(c).
 *
...
import { laneOf as laneFromRoute } from '@/app/routes';
...
/** One tab: its id, what its trigger reads, and the body behind it. */
export interface LaneTabItem {
  id: string;
  label: ReactNode;
  body: ReactNode;
  /**
   * `false` unmounts the body while another tab is showing.
   *
   * The default is to keep it mounted, and that is the load-bearing half: a
   * pane that unmounts drops its SSE subscription, so glancing at another lane
   * silently costs every line the first one printed while you were away — and
   * the replay endpoint cannot fill that gap, because it is fetched once and
   * those lines are newer than it. Only a body with no subscription (the
   * aggregate Waiting card, drawn from the board) may opt out.
   */
  mounted?: boolean;
}

/**
 * The tab strip, once — the guts `SessionTabs` and the runs index's `LaneTabs`
 * both need and each used to spell out.
 *
 * They differ in exactly three ways: their tab IDS (`p7` on a run page,
 * `slug:p7` across plans, where two tabs reading "Phase 5" are two plans),
 * their LABELS, and the extra tabs one of them adds. None of those is the
 * force-mount rule, which is the part that is easy to get wrong and was
 * written out twice.
 */
    ...
    <Tabs value={value} onValueChange={onValueChange}>
      ...
      {items.map((item) =>
        item.mounted === false ? (
          ...
          <TabsContent key={item.id} value={item.id} forceMount hidden={value !== item.id}>
            {item.body}
          </TabsContent>
        ),
      )}
...
export function SessionTabs({
  ...
  // `?lane=p7` opens that tab. Read every render rather than seeded into
  // `useState`, so following a second link while the page is already mounted
  // moves the strip — an initial-state seed would silently ignore it. An
  // explicit click still wins, because `picked` is checked first.
  const asked = laneFromRoute(useRoute());
  ...
  const value =
    picked === 'waiting' && waiting.length
      ? 'waiting'
      : resolveTab(picked ?? (asked != null ? `p${asked}` : null), lanes);

  const items: LaneTabItem[] = [
    {
      id: 'run',
      label: (
        <>
      ...
      ? [
          {
            ...
            // The one body with nothing subscribed: it is drawn from the plan
            // detail already on the page, so unmounting it costs no lines.
            body: <WaitingPane rows={waiting} />,
            mounted: false,
          },
        ]
      : []),
  ...
  return <LaneTabStrip value={value} onValueChange={setPicked} items={items} />;
}
