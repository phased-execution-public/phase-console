...
export class SearchIndex {
  /**
   * Sparse by design: a document dropped by `update()` leaves an EMPTY slot,
   * never a stripped object. Ids are held by `postings` and `bySlug`, so the
   * slot has to stay; the document behind it does not.
   */
  private docs: (SearchDoc | undefined)[] = [];
  private postings = new Map<string, Set<number>>();
  private bySlug = new Map<string, number[]>();
  private nextId = 0;
  /**
   * Slots `update()` emptied, waiting to be handed out again.
   *
   * The array is sparse by design (above), but `nextId` only ever went UP, so
   * every re-index of a plan appended its whole document count to `docs` and
   * left the vacated slots behind forever. `Service.forget` re-indexes on every
   * watcher event, so on a busy corpus the array grew without bound while
   * `size` — which filters — went on reporting the truth and hid it.
   *
   * Reusing an id is safe precisely where it is produced: `update()` has
   * already dropped the id from `postings` and reset its `bySlug` list, so
   * nothing holds it. `rebuild()` clears the pool for the same reason it
   * resets `nextId`.
   */
  ...
  update(record: PlanRecord): void {
    ...
    for (const id of ids) {
      ...
      for (const token of new Set(tokenize(`${doc.title} ${doc.section} ${doc.text}`))) {
        ...
        if (!set) continue;
        set.delete(id);
        // An emptied key is not the same as an absent one. `search` reads
        // `postings.get(term)` and takes an empty Set as "no match" without
        // ever consulting `prefixMatch`, so a word whose last document just
        // went away would go on suppressing the prefix hits another plan
        // still has. Drop the key with the last posting.
        if (!set.size) this.postings.delete(token);
      }
      // Not a tombstone object — the slot goes empty. Ids must stay stable
      // (`postings` and `bySlug` hold them), but the document is dead weight
      // the moment its postings are gone, and `Service.forget` runs this on
      // every docs change. Keeping `{...doc, text:''}` retained a whole
      // corpus of stripped sections and counted every one of them in `size`,
      // the "documents indexed" figure: six documents read as thirty-six
      // after five watcher events. Both readers below already skip a falsy
      // slot, so nothing else changes.
      this.docs[id] = undefined;
      // Nothing holds this id any more — its postings are gone above and its
      // `bySlug` list is reset below — so the slot goes back in the pool
  ...
  /**
   * Slots allocated, live or free — the figure `size` deliberately hides.
   *
   * `size` counts documents and is what a person wants; this counts the array
   * behind them, and is the only way a test can tell a re-index that reused its
   * ids from one that leaked them. Without it the leak is invisible: `size` was
   * correct throughout.
   */
  get capacity(): number { return this.docs.length; }
}
