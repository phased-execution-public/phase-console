...
export type PushMessage = {
  ...
  /**
   * The buttons on the notification, and the capability to press them.
   *
   * `actions` is what the worker hands to `showNotification`; `callback` is the
   * signed, expiring token it posts back to `/api/push/action` when one is
   * pressed. They travel together and are meaningless apart — see
   * `push/actions.ts` for why the payload names an inbox item rather than an
   * endpoint. Absent on every notification that is only worth opening.
   */
...
export async function deliver(
  ...
  try {
    ...
    // Named, because `status: 0` otherwise reads as "not the subscription's
    // fault" downstream — and a key that cannot be encrypted to never will be.
    return { kind: 'failed', status: 0, reason: 'encrypt', detail: `encrypt: ${(error as Error).message}` };
  }
