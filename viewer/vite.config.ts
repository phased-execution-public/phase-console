...
export default defineConfig({
  ...
  test: {
    ...
    css: false,
    // Vitest's stock 5000 ms, raised — the client half of what `91a8db2` did
    // for the server suite, and for the same measured reason.
    //
    // The FIRST test in a file pays for the lazy module graph its `mount()`
    // pulls in (the dialog behind `lazy-launch-dialog.tsx`, Radix, the whole
    // run-setup tree), transformed on demand INSIDE that test's own budget.
    // One test file per core is the default, so on a loaded machine the import
    // loses the race: phase 8's verification went red twice at load average 57
    // on 14 cores, both times on the first test of a file, at 5.1–5.6 s — and
    // on a DIFFERENT set of files each time (`run-setup`/`detail`/
    // `automation-coverage` once, `launch-dialog`/`launch-flow`/
    // `launch-shell.phone`/`reachability` the next). Unloaded, the same tree
    // is 170 files / 2589 tests green in 51 s.
    //
    // This is a liveness bound on the harness, never an assertion about the
    // app: not one test here claims a latency, a passing test never waits for
    // its timeout, a wrong expectation still fails identically, and a genuine
