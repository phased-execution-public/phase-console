 ...
 * it. So a card promising "the exact line for this console" composed a line
 * that turned MCP registration OFF, and `missing` (derived from this same
 * array) never listed `mcp` among the capabilities the console lacks — telling
 * an operator who wanted to turn MCP on that they already had everything.
 */
export const CAPABILITIES = [
  ...
  ['allowMcp', '--allow-mcp', 'mcp'],
  ['allowWebhooks', '--allow-webhooks', 'webhooks'],
] as const;
      ...
      <CardBody className="flex flex-col gap-3">
        ...
        <div>
          <div className="text-2xs uppercase tracking-wide text-ink-faint">
            {os} — run it in a terminal, Ctrl-C stops it
          </div>
        ...
        <div className="flex flex-col gap-1 text-2xs text-ink-faint">
          <span>
            <strong className="text-ink-muted">{otherUnix}:</strong> the same line works — the paths it shows
            are this machine's install, so run it where the console is installed.
          </span>
          <span>
            <strong className="text-ink-muted">Windows:</strong> Phase Console runs on macOS and Linux only —
            on Windows, install it inside WSL and run the line above in your WSL shell (<code>wsl</code>, then
            paste it). There is no native <code>cmd.exe</code> form.
          </span>
        </div>
        ...
        {state ? (
          missing.length ? (
            ...
            <p className="text-sm text-ink-muted">This console already runs with every capability.</p>
          )
        ) : null}

        <p className="text-2xs text-ink-faint">
          These flags are deliberately not composed: <code>--host</code> (the console binds localhost,
          always), <code>--scripts</code> and <code>--log-file</code>
          {/* A FRAGMENT, not a bare tail: prettier pulls loose JSX text up onto
              the marker line above, which is how the free card lost this whole
              sentence once already. An element is a node prettier keeps put. */}
          <> — the guide&apos;s reference table covers them.</>
        </p>
      </CardBody>
    </Card>
  );
