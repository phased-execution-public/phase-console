...
describe('the commands', () => {
  ...
  it('sends Freeze all to the board and fires nothing on the way', async () => {
    // Phase 19. Freezing stops every session on the machine mid-token, so it
    // is the `do:shutdown` case: three typed letters must not be able to do it.
    // The palette NAVIGATES to the board, where the button and its confirm
    // live, and `?view=board` is there because landing on the table would land
    // beside no control at all.
    const { onNavigate } = open('', { allowRun: true });
    fireEvent.click(await rowNamed('Freeze the whole console…'));
    expect(onNavigate).toHaveBeenCalledWith('#/runs?view=board');
  });

  it('offers no Freeze all on a console that cannot run anything', async () => {
    // A palette is a list of things you can do. The fleet doors are behind
    // `--allow-run`, so a row for them here would be a row that 403s.
    open('', { allowRun: false });
    expect(await rowNamed('Runs')).toBeTruthy();
    expect(screen.queryByText('Freeze the whole console…', { selector: 'span' })).toBeNull();
  });

  it('changes the density from here, and the change reaches the document', async () => {
    // The palette's claim is that it reaches everything, and until 4.0 that was
    // false for exactly one preference — the one an operator changes while
    // looking at a table too loose to scan, which is to say while their hands
    // are already on the keyboard. `data-density` on the root is the whole
    // switch: every pad token hangs off it (design.md §4), so asserting the
    // attribute is asserting that the preference PROPAGATES, not merely that it
    // was stored.
    open('');
    fireEvent.click(await rowNamed('Density: Compact'));
    expect(document.documentElement.getAttribute('data-density')).toBe('compact');

    fireEvent.click(await rowNamed('Density: Comfortable'));
    // Comfortable is the default and the default is the ABSENCE of the
    // attribute, never `data-density="comfortable"` — a second spelling of one
    // state is a thing for two selectors to disagree about.
...
describe('the way in', () => {
  ...
  /**
   * Ctrl-K is kill-to-end-of-line in every shell, and the console ships a
   * terminal. xterm's hidden helper textarea bubbles the keystroke to this
   * window listener and `pane.tsx` installs no `attachCustomKeyEventHandler`,
   * so the palette opened ON TOP of the shell and ate the shortcut — in the
   * one surface that owns its whole keyboard.
   */
  it('leaves Ctrl-K to a focused terminal, and still opens on ⌘K there', () => {
    const onNavigate = vi.fn();
    const client = new QueryClient(queryClientConfig);
    render(
      <QueryClientProvider client={client}>
    ...
    // The real shape: xterm's helper textarea, inside the `.phase-term` host.
    ...
    fireEvent.keyDown(helper, { key: 'k', ctrlKey: true });
    expect(onNavigate).not.toHaveBeenCalled();

    // ⌘K is not a terminal chord anywhere, so it stays the way in.
    fireEvent.keyDown(helper, { key: 'k', metaKey: true });
    expect(onNavigate).toHaveBeenCalledWith('#/now?k=');

    term.remove();
  });

  it('still opens on Ctrl-K away from a terminal', () => {
    const onNavigate = vi.fn();
    const client = new QueryClient(queryClientConfig);
    render(
      <QueryClientProvider client={client}>
  ...
  /**
   * The three overlays were the only surfaces here with no axe pass, and they
   * are the ones most likely to fail one: an overlay has to own focus, name
   * itself and be escapable, so a missing accessible name or a listbox with no
   * owner goes unnoticed precisely here. Mounted OPEN, with plans and runs in
   * context, so what is asserted is what an operator meets.
   */
  it('opens with no axe violations, with results in it', async () => {
