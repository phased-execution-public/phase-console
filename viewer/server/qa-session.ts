 ...
 * Re-exported from `shared/plan-vocab.js`, which owns the four words.
 */
export { QA_RESULTS };
...
export type QaFacts = QaRequest & {
  ...
  previous?: { result: string; report?: string; round?: number };
  /** Which round this review is — 1 unless a verdict is already recorded. */
  round?: number;
   ...
   *
   * `workRoot` rides along when that run has a checkout of its OWN — an
   * isolated run's commits are on `pe/<slug>` in a console-managed worktree,
   * while the console's own root is on whatever it was on. Naming the branch is
   * not enough there: `git checkout pe/<slug>` in the shared root is REFUSED,
   * because git allows a branch one working tree — so a reviewer told only the
   * branch would try, fail, and review the tree it was standing in. The launch
   * puts the session's cwd in that worktree instead, which is the one place the
   * phase's work exists.
   */
  gitStrategy?: { branch: string; workRoot?: string; mounts?: string[] };
};
...
function intro(facts: QaFacts): string {
  ...
  if (facts.gitStrategy?.workRoot && facts.gitStrategy.mounts?.length) {
    lines.push(
      '',
      `The phase's work lands on the plan's branch \`${facts.gitStrategy.branch}\`, in the`,
      "run's isolated workspace this session is ALREADY in — a MIRROR holding one",
      'worktree per scoped repository, each on that branch:',
      `    ${facts.gitStrategy.workRoot}`,
      `    (${facts.gitStrategy.mounts.join(', ')})`,
      'Review and run the tests inside those repositories. Do not check that branch out',
      'anywhere else — git allows a branch one working tree per repository, and these',
      'are the ones that have it.',
    );
  } else if (facts.gitStrategy?.workRoot) {
    lines.push(
      '',
      `The phase's work lands on the plan's branch \`${facts.gitStrategy.branch}\`, in the`,
      "console-managed worktree this session is ALREADY in:",
      `    ${facts.gitStrategy.workRoot}`,
      'Review it here and run the tests here. Do not check that branch out anywhere else —',
      'git allows a branch one working tree, and this is the one that has it.',
    );
  } else if (facts.gitStrategy) {
    lines.push(
      '',
...
function recording(facts: QaFacts): string {
  return [
    ...
    `    --report ${facts.reportArg}${facts.round && facts.round > 1 ? ` --round ${facts.round}` : ''}`,
    '',
...
/**
 * The instruction that asks a phase's OWN session for its owed QA verdict.
 *
 * One source, two callers: the ladder's `qa-verdict` rung (`service-runs.ts`
 * `vehicleForRung`) and the runner's at-finish dispatcher
 * (`runner-attempt.ts` `maybeQaVerdict`). The independence comes from the
 * fresh-context SUBAGENT the session dispatches, not from the session being a
 * different one — SKILL.md §QA. Real slug, phase and report path — never
 * placeholders: a resumed session is mid-conversation and will paste what it
 * is given.
 */
export function qaVerdictInstruction(
  slug: string, phase: number, round: number, report: string,
  /**
   * The verdict already on file, when there is one.
   *
   * The opening sentence asserted "your phase has NO recorded verdict", which is
   * true of the `qa-pending` chase this function was written for and FALSE of a
   * `qa-rerun` — whose whole premise is that a verdict exists and is being
   * reviewed again. A zero-context fresh reviewer takes an opening sentence as
   * fact, so it must be one. (QA round 2, L5.)
   */
  recorded?: string,
): string {
  const opening = recorded && recorded !== 'pending' && recorded !== 'none'
    ? `This plan gates on QA and your phase's recorded verdict is \`${recorded}\`, which is holding every `
      + 'phase that depends on it. You are being asked to review it AGAIN. '
    : 'This plan gates on QA and your phase has NO recorded verdict, so it is holding every phase that '
      + 'depends on it. ';
  return opening
    + 'Dispatch a FRESH-CONTEXT QA subagent over this phase now — get its brief with '
    + `\`bash scripts/phase-graph.sh ${slug} --qa-prompt ${phase}\` — and then record what it finds with `
    + `\`bash scripts/qa-record.sh ${slug} ${phase} <pass|fail|waived> --report ${report} --round ${round}\`. `
    + (round > 1
      ? `This is QA round ${round} on this phase: write the report to its OWN file (the name above), `
        + 'never over the earlier round\'s. '
      : '')
...
/**
 * Re-exported so the callers that had it here keep working. The definition —
 * and every rule about which round is next — lives in `qa-round.ts`, which is
 * the ONLY place in `server/` allowed to build one of these paths. Four QA
 * rounds went by fixing separate copies of that decision one at a time.
 */
export { qaReportPath, nextQaRound } from './qa-round.ts';
