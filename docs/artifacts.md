...
| **Gate approvals** *(only once a gate is cleared)* | `docs/handoffs/<slug>/gate-status.md` | Per-phase gate clearances written by `gate-approve.sh` — the console's Gate card, an AI session that verified an `ai` gate, or a hand run. `--gate-status` honours an approved row for **every** gate kind. A separate file from the QA table on purpose: recording an approval must never flip QA gating on. |
