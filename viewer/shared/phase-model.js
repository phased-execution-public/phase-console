 ...
 *
 * The MEMBERS come from `status-vocab.js`'s `BOARD_BUCKETS` — the engine's
 * bucket vocabulary — so a sixth board word cannot be added there and go
 * silently missing here. Only the ORDER is decided locally, and
 * `test/vocab-owners.test.ts` holds it to being a permutation of the owner.
 */
const OPERATOR_RANK = /** @type {Record<string, number>} */ ({
  'in-progress': 0,
  ready: 1,
  stuck: 2,
  waiting: 3,
...
export const BOARD_ORDER = Object.freeze(
  [...BOARD_BUCKETS].sort((a, b) => (OPERATOR_RANK[a] ?? 99) - (OPERATOR_RANK[b] ?? 99)),
);
