 ...
 *
 * Twelve, down from forty in Phase 8, because the list no longer scrolls
 * inside itself: a second vertical scroller nested in the dialog's own eats
 * the flick meant for the outer one (`components/ui/table.tsx` bans the same
 * arrangement for tables, and the Phase 6 register measured it here — 288px
 * over 2,154px of rows). The list now lays out in the dialog's scroller, so
 * the cap is what keeps a 60-skill group from being a screen and a half of
 * rows; the search is the way to the rest, and the sentence under the cap
 * says so.
 */
...
export function SkillPicker({
  ...
  //
  // Minus what the PLAN names, the same subtraction `selectable` already makes.
  // A QA review seeds `chosen` from the plan's own skills, so a plan with
  // `Skills (every session): design-review, browse` opened reading
  // "2 chosen · 2 from the plan" and listed the same two ids twice — once as
  // immutable plan chips saying "edit the plan if they are wrong", and again
  // directly below as removable ones. Removing a chip that the plan puts back
  // into every boot prompt does nothing an operator can see.
  const fromPlan = new Set(planSkills);
  const chosenSkills = chosen
    .filter((id) => !fromPlan.has(id))
    .map((id) => skills.find((s) => s.id === id) ?? { id, description: 'not installed on this machine' });
  ...
  return (
    <details className="rounded-lg border border-rule bg-surface">
      ...
      <div className="border-t border-rule px-3 py-3">
        {planSkills.length > 0 && (
          <div className="mb-3">
            <p className="mb-1.5 max-w-prose text-2xs text-ink-muted">
          ...
          <div className="mb-3 flex flex-wrap gap-1.5" role="group" aria-label="Chosen skills">
            {chosenSkills.map((s) => (
              <button
                ...
                onClick={() => toggle(s.id)}
                className="inline-flex items-center gap-1 rounded-sm border border-action/50 bg-action/10 px-1.5 py-0.5 font-mono text-2xs text-action disabled:opacity-50 [@media(hover:none)]:min-h-(--tap-min) [@media(hover:none)]:px-3"
              >
                {s.id}
                {byDefault.has(s.id) && (
        ...
        {/* No scroller of its own — see SHOWN_PER_GROUP. Each row takes the
            thumb floor as its BOX (`tap-row`): a row in a stack of rows cannot
            use the checkbox's overlay floor, which would reach into the rows
            above and below it. */}
        <div className="mt-2">
          {groups.length ? (
            groups.map(([group, list]) => (
              <div key={group}>
                <div className="py-1 text-2xs tracking-wide text-ink-muted uppercase">
                  {SOURCE_LABEL[group] ?? group} · {list.length}
                </div>
                {list.slice(0, SHOWN_PER_GROUP).map((skill) => (
                  <label
                    key={skill.id}
                    className="tap-row flex items-baseline gap-2 py-0.5 text-2xs hover:bg-surface-raised"
                  >
                    <Checkbox
                ...
                {list.length > SHOWN_PER_GROUP && (
                  <p className="py-1 text-2xs text-ink-muted">
            ...
            <p className="py-2 text-2xs text-ink-muted">
              Nothing matches {query ? <b>“{query}”</b> : 'that filter'}.
              {source && (
                <>
                  {' '}
                  Try{' '}
                  <button type="button" className="tap-line underline" onClick={() => setSource('')}>
                    all sources
                  </button>
                  .
                </>
        ...
        <p className="mt-3 max-w-prose text-2xs text-ink-muted">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        '[@media(hover:none)]:min-h-(--tap-min) [@media(hover:none)]:px-3',
