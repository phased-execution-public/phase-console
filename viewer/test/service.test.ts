...
test('service composes plans, detail and portfolio', { skip: !available && 'no source directory' }, async () => {
  ...
  // The lint is no longer part of what a plan-detail READ waits for.
  //
  // `validate.sh` walks every handoff file, serially, and each walk re-parses
  // the whole plan — 11.27 s on the live 22-handoff plan, paid before the page
  // could paint a table the lint does not appear in. Nothing about it was
  // removed: it is still the engine's own answer, still cached by revision,
  // the route below computes it, it is pushed as `plan:lint` when it lands,
  // and the next open of the plan carries it. `test/read-path.test.ts` is the
  // witness that the read itself no longer spawns it.
  assert.ok(await service.lint(withGraph.slug), 'lint should still run — behind its own route');
  assert.ok((await service.detail(withGraph.slug))!.lint, '…and ride along on the next open');
