...
/**
 * Every keychain service name this server's secrets live under.
 *
 * The prefix is a first-class thing rather than an implicit consequence of the
 * template below, because `deleteAll` has to enumerate by it: a ref the
 * operator removed from a server's `secretRefs` is no longer walkable from the
 * metadata, and a keychain item nobody can name is one nobody can delete.
 */
export function mcpKeychainPrefix(id: string): string {
  return `phase-console-mcp-${id}-`;
}

/** Our own keychain service for one server's secrets. */
export function mcpKeychainService(id: string, ref: string): string {
  return `${mcpKeychainPrefix(id)}${ref}`;
}
...
export class McpCredentials {
  ...
  async store(id: string, ref: McpSecretRef, secret: string): Promise<void> {
    ...
    if (this.platform === 'darwin') {
      await keychainStore(this.exec, mcpKeychainService(id, key), secret);
      return;
    }
   ...
   *
   * The declared refs are not the whole story, and walking only them is how a
   * removed server's credentials outlived it. `secretRefs` describes what the
   * server needs TODAY: edit a server from `header:Authorization` to
   * `header:X-Api-Key`, or drop a ref that is no longer required, and the old
   * keychain item is instantly unreachable from the metadata — still holding a
   * live bearer token, and now with nothing left that can name it. Removing the
   * server then deleted the row, the file, and none of that.
   *
   * So on darwin the authority is the KEYCHAIN, enumerated by our own service
   * prefix: everything this console ever stored for this id, whether or not the
   * metadata still remembers asking for it. The declared refs are still swept
   * first, so a keychain we cannot enumerate degrades to the old behaviour
   * rather than to nothing.
   *
   * Elsewhere the file holds every key in one place already, so unlinking it is
   * the same guarantee.
   */
  async deleteAll(meta: McpServerMeta): Promise<void> {
    ...
    for (const service of await this.keychainServicesFor(meta.id)) {
      try {
        await this.exec('security', ['delete-generic-password', '-s', service]);
      } catch { /* already gone, or never ours — same outcome */ }
    }
  }

  /**
   * Service names under this server's prefix, from the keychain itself.
   *
   * `dump-keychain` without `-d` prints metadata only — no secret leaves the
   * keychain and no unlock prompt is raised — one `"svce"<blob>="…"` line per
   * item. A keychain that will not enumerate answers with nothing, which leaves
   * `deleteAll` exactly as good as it was before rather than failing a removal.
   */
  private async keychainServicesFor(id: string): Promise<string[]> {
    const prefix = mcpKeychainPrefix(id);
    let stdout: string;
    try {
      ({ stdout } = await this.exec('security', ['dump-keychain']));
    } catch {
      return [];
    }
    const found = new Set<string>();
    for (const match of stdout.matchAll(/"svce"<blob>="((?:[^"\\]|\\.)*)"/g)) {
      const service = match[1].replace(/\\(.)/g, '$1');
      if (service.startsWith(prefix)) found.add(service);
    }
    return [...found];
  }
