...
/** The labels, from the one module that owns them. */
export const PROFILE_LABEL: Record<PermissionProfile, string> = PROFILE_LABELS;
...
/**
 * What each choice does on a QA fail — the one difference between the two
 * modes that the labels never named. Measured (2026-09-07): "Stop and ask me"
 * did not stop on a failed review at all; now it parks the run on that phase
 * with a `needs-human` halt naming the report, and "Keep going" leaves the
 * failed phase to the recovery ladder while the phases that do not depend on
 * it carry on — the documented design, said out loud where it is chosen.
 */
export const AUTONOMY_HELP: Record<Autonomy, string> = {
  'halt-on-everything':
    'A QA fail (or a verdict still pending when the round is up) parks the run on that phase with a needs-human halt naming the report — nothing else runs until you say so.',
  'keep-going':
    'A QA fail does not stop the run: the failed phase is fixed by the recovery ladder while independent phases carry on, and its dependents stay held until a pass is recorded.',
};
