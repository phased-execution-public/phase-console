...
import { Badge, MonoId, SectionHeading } from '@/components/ui';
...
export interface LiveLanesProps {
  ...
  /**
   * The admission queue. Optional because this section renders before
   * `/api/queue` has answered, and a band with no waiting list is a truthful
   * band — it is the ABSENCE of a queue, not a claim that nothing is waiting.
   */
...
export function LiveLanes({
  ...
  // One band per plan, ordered by its own worst lane — see `ops-model.ts`.
  const bands = useMemo(() => swimlanes(lanes, queue), [lanes, queue]);
  const waiters = useMemo(() => waitersByLane(queue), [queue]);
  const waitingCount = bands.reduce((n, band) => n + band.waiting.length, 0);
      ...
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        {lanes.length > 0 && (
          <span className="text-2xs text-ink-faint">
            ...
            {bands.length > 1 ? ` across ${plural(bands.length, 'plan')}` : ''}
            {queued ? ` · ${queued} queued` : ''}
            {stalled ? ` · ${stalled} not moving` : ''}
            {waitingCount ? ` · ${waitingCount} waiting for a scope` : ''}
          </span>
        )}
      </div>

      {bands.length === 0 ? (
        <NoBands headline={idle.headline} detail={idle.detail} />
      ) : (
        <div className="flex flex-col gap-2">
          {bands.map((band) => (
            <SwimlaneBand key={band.slug} band={band} tails={tails} waiters={waiters} allowRun={allowRun} />
          ))}
        </div>
      )}
          ...
          >
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-0.5 flex flex-wrap items-center gap-1.5 text-2xs text-ink-muted">
                ...
                <MonoId id={session.sessionId} className="text-ink-faint" />
              </span>
            </span>
