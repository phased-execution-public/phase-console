...
export default function NowView({ route }: ViewProps) {
  ...
  // The admission queue — what holds a scope and what waits on it. Same
  // stale-server guard as every other run-reading query on this page.
  const { data: queue } = useQueue(runsEnabled);
    ...
    <Page title="Now" className="pb-6">
      <div className="flex flex-col gap-4">
        ...
        <PlansInFlight
          plans={stripPlans}
          details={bySlug}
          loading={detailsLoading || isPending}
          focused={focus === FOCUS_KEYS.plans}
        />
      </div>
    </Page>
  );
