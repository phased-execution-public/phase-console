...
describe('settings', () => {
  ...
  it('renders one section when the address names one, and not the others', async () => {
    const { default: SettingsView } = await import('./index');
    mount(<SettingsView route={at('essentials')} />);

    expect(await screen.findByRole('heading', { name: 'Essentials', level: 2 })).toBeTruthy();
    ...
    // The Engine card is Essentials'; the process facts are This instance's.
    expect(screen.queryByText('Server code')).toBeNull();
  });

  it('redirects a retired section id rather than resolving it quietly', async () => {
    const { redirectTarget } = await import('@/app/routes');
    // Three ids are live in bookmarks and in payloads the server minted before
    // this build — including its own `/#/settings/alerts` webhook link. The
    // address bar must end up saying where the page actually is.
    expect(redirectTarget(at('general'))).toBe('settings/essentials');
    expect(redirectTarget(at('alerts'))).toBe('settings/notifications');
    expect(redirectTarget(at('process'))).toBe('settings/instance');
    // A live id is not a redirect, and neither is the bare index.
    expect(redirectTarget(at('essentials'))).toBeNull();
    expect(redirectTarget(at())).toBeNull();
  });

  it('finds a section in the palette by what is INSIDE it, not by its name', async () => {
    const { settingsActions } = await import('@/app/command/actions');
    const rows = settingsActions();
    const { SETTINGS_SECTIONS } = await import('./nav');
    expect(rows).toHaveLength(SETTINGS_SECTIONS.length);
    // The whole point: `quiet hours` is a control, not a section, and until
    // Phase 7 the palette indexed only the eight destination names — so the
    // only route to a setting was to guess which noun held it.
    const terms = (id: string) => rows.find((row) => row.id === `settings:${id}`)!.keywords ?? '';
    expect(terms('notifications')).toMatch(/quiet hours/);
    expect(terms('permissions')).toMatch(/deny/);
    expect(terms('instance')).toMatch(/restart/);
    expect(terms('essentials')).toMatch(/start command/);
  });
...
/* ------------------------------------------------------------------ *
 * Notifications — the section the notifications page became
 * ------------------------------------------------------------------ */
...
describe('the notifications section', () => {
  it('renders the settings cards and points at the drawer', async () => {
    const { default: SettingsView } = await import('./index');
    mount(<SettingsView route={at('notifications')} />);

    expect(await screen.findByRole('heading', { name: 'Notifications', level: 2 })).toBeTruthy();
