...
export function HelpSheet({ route }: { route: Route }) {
  ...
  return (
    ...
    >
      ...
      <SheetContent side={phone ? 'bottom' : 'right'} title="Help" showTitle bodyClassName="p-0">
        {open && (
          <div className="flex flex-col gap-4 p-4">
            ...
            >
              <SectionPanel
                ...
                hrefFor={(cardId) => helpHref(section.id, cardId, route)}
              />
            </Suspense>
          </div>
        )}
...
export function ConsolePosture() {
  ...
  return (
    <p className="flex items-center gap-2 text-sm text-ink-muted">
      <span aria-hidden className={cn('size-2 shrink-0 rounded-full', on ? 'bg-done' : 'bg-ink-faint/50')} />
      {text}
    </p>
  );
}
