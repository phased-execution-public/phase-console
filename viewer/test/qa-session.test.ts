...
const {
  MAX_QA_PROMPT_BYTES, QA_RESULTS, isVerdict, parseQaRequest, qaKey, qaLabel, qaPrompt, qaVerdictInstruction,
...
test('a QA session is minted fresh, named for its target, and linked back to it', () => {
  ...
  // `report` rides along so the Sessions destination can say which file this
  // review writes — and can link it the moment it exists. Until rounds landed
  // that path lived only inside the prompt.
  assert.deepEqual(meta?.qa, { slug: 'alpha', phase: 2, report: 'reports/phase-02-qa.md' });
...
test('the verdict snapshot rides on the session, so its exit can be judged', () => {
  ...
  if (built.ok) {
    assert.deepEqual(built.launch.meta?.qa, {
      ...
      report: 'reports/phase-02-qa.md',
    });
  }
});

test('a later round rides on the session too, with the report it must not overwrite', () => {
  // The service numbers the round and picks the filename (`resolveQa`); the
  // mint only carries them. Both halves matter to a reader: `beforeReport` is
  // the round that HAS been written, `report` is the one about to be — and
  // before this they were the same string, which is how a second review used
  // to destroy the first one's report.
  const built = buildAgentLaunch({ intent: 'qa' }, ctx({
    qa: facts({
      round: 2,
      reportArg: 'reports/phase-02-qa-round2.md',
      previous: { result: 'fail', report: 'reports/phase-02-qa.md', round: 1 },
    }),
  }));
  assert.equal(built.ok, true);
  if (!built.ok) return;
  assert.equal(built.launch.meta?.qa?.round, 2);
  assert.equal(built.launch.meta?.qa?.report, 'reports/phase-02-qa-round2.md');
  assert.equal(built.launch.meta?.qa?.beforeReport, 'reports/phase-02-qa.md');
  assert.notEqual(built.launch.meta?.qa?.report, built.launch.meta?.qa?.beforeReport);
});
...
test('the launcher numbers the round from the LEDGER, and agrees with the engine', async () => {
  // QA round 3, F1 — a Critical. The launcher took the round from the STATUS
  // row, and the moment that row says `pending` it has forgotten every round
  // behind it: recorded fail-1, fail-2, then pending, the engine said round 3
  // while the launcher said round 1 and named a report two reviews old. One
  // brief, contradicting itself, pointing at a file already on disk.
  const { root, cleanup } = scratch({ handoffs: [1, 2] });
  try {
    const dir = join(root, 'docs', 'handoffs', 'alpha');
    mkdirSync(join(dir, 'reports'), { recursive: true });
    writeFileSync(join(dir, 'reports', 'phase-02-qa.md'), '# round 1', 'utf8');
    writeFileSync(join(dir, 'reports', 'phase-02-qa-round2.md'), '# round 2', 'utf8');
    writeFileSync(join(dir, 'test-status.md'), [
      ...
      '|------:|------:|--------|--------|----------|',
      '| 2 | 1 | fail | reports/phase-02-qa.md | 2026-09-01 |',
      '| 2 | 2 | fail | reports/phase-02-qa-round2.md | 2026-09-02 |', '',
    ].join('\n'), 'utf8');
    ...
    assert.equal(resolved.facts.round, 3, 'the ledger holds two rounds, so this is the third');
    assert.equal(resolved.facts.reportArg, 'reports/phase-02-qa-round2.md'.replace('round2', 'round3'));
    // …and the brief it renders never names an occupied file.
    const text = qaPrompt(resolved.facts);
    assert.ok(text.includes('reports/phase-02-qa-round3.md'));
    assert.ok(!text.includes('--report reports/phase-02-qa.md'), text.slice(0, 400));
  } finally { cleanup(); }
});

test('the launcher steps past a report already on disk that no row mentions', async () => {
  // QA round 3, F2. "Recorded nothing" is a first-class outcome — the runner
  // journals `phase.qa-session-skipped` for it — so a reviewer can leave a
  // report with no row anywhere, and numbering from the ledger alone would hand
  // the next reviewer that exact filename.
  const { root, cleanup } = scratch({ handoffs: [1, 2] });
  try {
    const dir = join(root, 'docs', 'handoffs', 'alpha');
    mkdirSync(join(dir, 'reports'), { recursive: true });
    // Nothing recorded at all — and yet round 1's and round 2's files exist.
    writeFileSync(join(dir, 'reports', 'phase-02-qa.md'), '# written, never recorded', 'utf8');
    writeFileSync(join(dir, 'reports', 'phase-02-qa-round2.md'), '# same', 'utf8');

    const svc = service(root);
    const resolved = await svc.resolveQa({ slug: 'alpha', phase: 2 });
    assert.equal(resolved.ok, true);
    if (!resolved.ok) return;
    assert.equal(resolved.facts.round, 3);
    assert.equal(resolved.facts.reportArg, 'reports/phase-02-qa-round3.md');
  } finally { cleanup(); }
});
...
test('the verdict instruction says the dispatched reviewer is work inside THIS turn', () => {
  // Measured (phase-console-commerce P8): a session dispatched the round-2
  // reviewer and ended its turn "to pick it up when it returns". Nothing picks
  // it up — the subagent dies with the turn — so the verdict was never
  // recorded, the rung was scored a failure, and the plan parked on a human
  // errand for a defect that was already fixed.
  const text = qaVerdictInstruction('alpha', 7, 2, 'reports/phase-07-qa-round2.md');
  assert.match(text, /inside (your|this) turn/i);
  assert.match(text, /wait for it/i);
  assert.match(text, /ends? .*(while|before) .*(records nothing|nothing is recorded)/i);
});

test('a QA session is handed the task channel, so its list reaches the console', () => {
  // The pty reviewer had no `PE_TASKS_FILE`, so `phase-tasks.sh` fell through
  // to the unsupervised inbox — which nothing read. Named here, the inbox file
  // for (slug, phase) is where the Sessions page now folds the list from.
  const built = buildAgentLaunch({ intent: 'qa' }, ctx({ root: '/repo/alpha-root' }));
  assert.equal(built.ok, true);
  if (!built.ok) return;
  assert.match(built.launch.env?.PE_TASKS_FILE ?? '', /\/alpha\/tasks\/phase-02\.ndjson$/);
});
